...
@section('menu_title')
    <li class="d-flex align-items-center">
        <div class="d-inline-block d-flex mr-auto align-items-center ml-1 lvl-1">
            <h4 class="my-0 me-2 menu-title text-white">Crear plantilla email</h4>
        </div>
    </li>
@endsection
...
@section('content')
    ...
    <form id="classic-builder-form" action="{{ route('manager.layouts.store') }}" method="POST"
          class="ajax_upload_form form-validate-jquery email-template">
            ...
            <div class="col-md-3 pr-0 pb-0 sidebar pr-4 pt-4 pl-4" style="overflow:auto;background:#f5f5f5">
                ...
                @if (count($tags) > 0)
                    <div class="tags_list">
                        ...
                        @foreach($tags as $tag)
                            @if (!$tag["required"])
                                <a draggable="false" data-popup="tooltip" title='Haga clic para insertar etiqueta'
                                   href="javascript:;" class=" insert_tag_button" data-tag-name="{{ $tag["name"] }}">
                                    {{ $tag["name"] }}
                                </a>
                            @endif
                        @endforeach
