...
@section('content')
    ...
    @include('managers.components.card', ['title' => 'Usuarios'])
                ...
                <div class="col-md-12 col-xl-12">
                    <form class="position-relative form-search" action="{{ request()->fullUrl() }}" method="GET">
                        <div class="row justify-content-between g-2 ">
                            ...
                            <div class="col-auto">
                                <a href=" {{ route('manager.campaigns.maillists.subscribers.create' , $list->uid) }}" class="btn btn-primary">
                ...
                <table class="table search-table align-middle text-nowrap">
                    ...
                    @foreach ($subscribers as $key => $item)
                        <tr class="search-items">
                            <td>
                                <span class="usr-email-addr" >{{ Str::words( Str::upper(Str::lower($item->subscriber->firstname . ' ' . $item->subscriber->lastname)), 12, '...')  }}</span>
                            </td>
                            <td>
                                <span class="usr-email-addr" >{{ $item->subscriber->email  }}</span>
                            </td>
                            <td>
                                <span class="badge {{ $item->available == 1 ? 'bg-light-secondary  text-primary' : 'bg-light-secondary  text-primary' }}  rounded-3 py-2 text-primary fw-semibold fs-2 d-inline-flex align-items-center gap-1">
                                           {{ $item->available == 1 ? 'Publico' : 'Oculto' }}
                                </span>
                            </td>
                            <td>
                            ...
                            <td class="text-left">
                                <div class="dropdown dropstart">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <li>
                                            <a class="dropdown-item d-flex align-items-center gap-3" href="{{ route('manager.subscribers.lists', $item->uid) }}">
                                                Listado
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item d-flex align-items-center gap-3" href="{{ route('manager.subscribers.logs', $item->uid) }}">
                                                Logs
                                            </a>
                                        </li>
                                        <li>
                                        ...
                                        <li>
                                            <a class="dropdown-item d-flex align-items-center gap-3 confirm-delete" data-href="{{ route('manager.subscribers.destroy', $item->uid) }}">Eliminar</a>
                                        </li>
                                    </ul>
                                </div>
                            </td>
