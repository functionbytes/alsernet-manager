@extends('manager.layouts.core.backend_dark')
...
@section('menu_title')
    <li class="d-flex align-items-center">
        <div class="d-inline-block d-flex mr-auto align-items-center ml-1 lvl-1">
            <h4 class="my-0 me-2 menu-title text-white">{{ $template->title }}</h4>
...
@section('menu_right')
    ...
    <li class="d-flex align-items-center px-3">
        <button class="btn btn-primary"
                onclick="$('#classic-builder-form').submit()">{{ trans('messages.save') }}</button>
    </li>
    <li>
        <a href="javascript:;"
           onclick="parent.$('body').removeClass('overflow-hidden');parent.$('.full-iframe-popup').fadeOut()"
...
@section('content')
    <form id="classic-builder-form" action="{{ route('manager.templates.update', $template->uid) }}" method="POST"
          class="ajax_upload_form form-validate-jqueryz builder-classic-form">
        {{ csrf_field() }}
    ...
    <script>
        ...
        $(document).ready(function () {
            editor = tinymce.init({
                language: '{{ Auth::user()->getLanguageCode() }}',
                selector: '.template-editor',
                directionality: "{{ Auth::user()->text_direction }}",
