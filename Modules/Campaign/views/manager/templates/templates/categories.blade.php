...
@section('content')
    ...
    <form class="categories-form" action="{{ route('manager.templates.categories', ['uid' => $template->uid,]) }}"
          method="POST">

        <h5 class="mt-0 mb-2">Establecer categoría de plantilla - {{ $template->title }}</h5>
        <p class="mt-0 pb-2">Elija una o más categorías para su plantilla. Aviso: si su plantilla no pertenece a ninguna
            categoría, no será visible para los usuarios.</p>
