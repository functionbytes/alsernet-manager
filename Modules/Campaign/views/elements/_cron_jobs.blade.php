...
@if(!$valid || isset($show_all))
    @include('resources.views.helpers.form_control', [
        'type' => 'radio',
        'name' => 'queue_driver',
        'class' => '',
...
<div class="database-config-box">
    ...
    @if($valid || (isset($show_all) && !$errors->has('php_bin_path_invalid')))
        <div id="result_box">
            ...
            <div class="d-flex">
                <pre id="cronString" class="py-2 px-2 mb-0 pr-4 bg-light" style="font-size: 16px;">* * * * * <span
                            class="current_path_value">{!! $php_bin_path_value !!}</span> -q {{ base_path() }}/artisan schedule:run 2&gt;&amp;1</pre>
                <button type="button" class="btn btn-info copy-button rounded-0 rounded-end"><span
                            class="material-symbols-rounded">content_copy</span> {{ trans('messages.copy') }}</button>
            </div>
        </div>
    @endif
</div>
