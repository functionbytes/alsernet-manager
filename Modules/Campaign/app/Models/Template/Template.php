...
class Template extends Model
{
    ...
    {
        $category = TemplateCategory::findByUid($uid);
        // Category
        if ($category) {
    ...
    public function copy($attributes = [])
    {
        ...
        $copy->title = isset($attributes['title']) ? $attributes['title'] : $this->title;
    ...
    public function getBuilderAdminTemplates()
    {
        ...
        foreach ($templates as $template) {
            $result[] = [
                'name' => $template->name,
                'url' => route('manager.templates.builder.change.template', ['uid' => $this->uid, 'change_uid' => $template->uid]),
    ...
    public function changeName($title)
    {
