 ...
 * @property string|null $mail_subscribe
 * @property string|null $mail_unsubscribe
 * @property string|null $mail_daily
 ...
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist newQuery()
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist perPage($size, $callback)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist query()
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist search($keyword)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereAllSendingServers($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereAvailable($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereCreatedAt($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereCustomOrder($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereDefaultSubject($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereEmailDaily($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereEmailSubscribe($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereEmailUnsubscribe($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereFromEmail($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereFromName($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereId($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereRemindMessage($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereSendWelcomeEmail($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereSubscribeConfirmation($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereTitle($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereUid($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereUnsubscribeNotification($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|CampaignMaillist whereUpdatedAt($value)
 *
 * @mixin \Eloquent
 */
class CampaignMaillist extends Model
{
    ...
    public static $rules = [
        ...
        'from_name' => 'required',
        'mail_subscribe' => 'nullable|regex:"^[\W]*([\w+\-.%]+@[\w\-.]+\.[A-Za-z]{2,4}[\W]*,{1}[\W]*)*([\w+\-.%]+@[\w\-.]+\.[A-Za-z]{2,4})[\W]*$"',
        'mail_unsubscribe' => 'nullable|regex:"^[\W]*([\w+\-.%]+@[\w\-.]+\.[A-Za-z]{2,4}[\W]*,{1}[\W]*)*([\w+\-.%]+@[\w\-.]+\.[A-Za-z]{2,4})[\W]*$"',
        'mail_daily' => 'nullable|regex:"^[\W]*([\w+\-.%]+@[\w\-.]+\.[A-Za-z]{2,4}[\W]*,{1}[\W]*)*([\w+\-.%]+@[\w\-.]+\.[A-Za-z]{2,4})[\W]*$"',
    ];
    ...
    {
        return $this->hasMany(CampaignSegment::class, 'maillist_id');
    }
    ...
    {
        return $this->belongsToMany(Campaign::class, 'campaigns_lists_segments', 'maillist_id', 'campaign_id');
    }

    public function subscriberFields()
    {
        return $this->hasManyThrough('Modules\Campaign\Entities\SubscriberField', CampaignField::class);
    }
    ...
    public function export($progressCallback = null, $segment = null)
    {
        ...
        cursorIterate($query, $orderBy = 'campaigns_maillists_subscribers.id', $pageSize, function ($subscribers, $page) use ($writer, &$processed, &$total, &$failed, &$message, $progressCallback) {
    ...
    public function validateFieldsMap($map)
    {
        ...
        if (! in_array($emailFieldId, $fieldIds)) {
            throw new Exception(trans('messages.list.import.error.mail_missing'));
        }
        ...
        try {
            ...
            each_batch($results, config('app.import_batch_size'), false, function ($batch) use ($map, &$processed, &$failed, $total, &$overQuotaAttempt, &$message, $progressCallback, $invalidRecordCallback) {
                ...
                $insertToSubscribersSql = strtr(
                    ...
                    FROM (
                        SELECT tmp.%mail_field AS email, tmp.tags
                        FROM %tmp tmp
                        LEFT JOIN %subscribers main ON (tmp.%mail_field = main.email AND main.maillist_id = %list_id)
    ...
    public function updateOrCreateFieldsFromRequest($request)
    {
        ...
        foreach ($request->fields as $key => $item) {
            ...
            if ($this->getEmailField()->uid != $item['uid']) {
                ...
                // Check tag exsit
                $tag = CampaignField::formatTag($item['tag']);
        ...
        foreach ($request->fields as $uid => $item) {
            $field = CampaignField::findByUid($item['uid']);
            if (! $field) {
            ...
            if ($this->getEmailField()->uid != $field->uid) {
                // save exsit field
                $item['tag'] = CampaignField::formatTag($item['tag']);
                $field->fill($item);
                $field->save();
                ...
                if (isset($item['options'])) {
                    foreach ($item['options'] as $key2 => $item2) {
                        $option = new CampaignFieldOption($item2);
