...
function ProcesarProducto($data, $fila, $tipo)
{
    ...
    try {
        ...
        echo $idprodps . " " . $idproductattributeps . " " . $idprodpssinatributo . "\n";
...
function ProcesarModelo($data, $fila, $tipo)
{
    ...
    if ($tipo <= 2) {
        ...
        if ($idprodps != "") {
            ...
            // Mientras no se traduce, que vaya por lo menos rellenando en Español todos los demás idiomas
            foreach ([_PSALV_LANG_ID_ES_, _PSALV_LANG_ID_EN_, _PSALV_LANG_ID_FR_, _PSALV_LANG_ID_PT_, _PSALV_LANG_ID_DE_] as $id_lang) {
                $product->name[$id_lang] = ($actualizarnombre) ? str_replace("#","",$nombre) : $product->name[$id_lang];
                $product->link_rewrite[$id_lang] = Tools::link_rewrite($product->name[$id_lang]); // Parece que se quiere mantener el slug inicial. Sino, debería de usarse $nombre

                $product->description[$id_lang] = ($actualizardescripcion) ? $descripcion : $product->description[$id_lang];

                $product->meta_title[$id_lang] = ($actualizarnombre) ? $nombre . $sufixtitle : $product->meta_title[$id_lang];
                $product->meta_description[$id_lang] = ($actualizarnombre) ? $nombre . $sufixdescription[$id_lang] : $product->meta_description[$id_lang];
            }
...
try {
    ...
    */
            ...
            $url_trasaccion = 'http://223.1.1.22:9000/integracion/TransaccionPendiente/?destino=2&transaccion='.$transaccion;

            //$transaccion = "8.43.1334845"; //Lote: Conjunto de Barrera de Olor para Cultivos
            $transaccion = "11.41.384017";
...
echo "2\n";
                ...
                $contentitem = peticionget($url_trasaccion . "&limit=5000");
