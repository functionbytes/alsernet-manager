...
{block name="override_tpl"}
	...
	{else}
		...
		<div class="panel">
			...
			<p>{l s='Total missing expressions:' d='Admin.International.Feature'} <span class="badge">{l s='%d' sprintf=[$missing_langs|array_sum]}</p>
		</div>
				...
				{if !empty($newLang)}
					<div class="panel">
						<h3>
							...
							{if isset($missing_langs[$k])} <span class="label label-danger">{$missing_langs[$k]} {l s='missing' d='Admin.International.Feature'}</span>{/if}
						</h3>
						<div name="{$type}_div" id="{$k}-tpl" style="display:{if isset($missing_langs[$k])}block{else}none{/if}">
							<table class="table">
