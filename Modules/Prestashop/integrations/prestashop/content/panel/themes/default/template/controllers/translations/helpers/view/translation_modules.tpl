...
{block name="override_tpl"}
	...
	{else}
		...
		<div class="panel">
			...
			<p>{l s='Total missing expressions:' d='Admin.International.Feature'} <span class="badge">{l s='%d' sprintf=[$missing_langs]}</p>
		</div>
		...
		<form method="post" id="{$table}_form" action="{$url_submit|escape:'html':'UTF-8'}" class="form-horizontal">
			<div class="panel">
				...
				<div id="BoxUseSpecialSyntax">
					<div class="alert alert-warning">
						...
						<ul>
							<li>"{l s='There are [1]%replace%[/1] inventaries' html=true sprintf=['%replace%' => '%d', '[1]' => '<strong>', '[/1]' => '</strong>'] d='Admin.International.Help'}": {l s='"%s" will be replaced by a number.' sprintf=['%d'] d='Admin.International.Help'}</li>
			...
			{foreach $modules_langs as $theme_name => $selected_theme}
				...
				{foreach $selected_theme as $module_name => $module}
					...
					{foreach $module as $template_name => $newLang}
						{if !empty($newLang)}
							...
							{/foreach}
							{if $occurrences > 0}
								{$missing_langs_module = $occurrences}
							{else}
								{$missing_langs_module = 0}
							{/if}
							<div class="panel">
								<h3 onclick="$('#{$theme_name}_{$module_name}_{$template_name|replace:'.':'_'}').slideToggle();">{if $theme_name}{$theme_name} - {/if}{$template_name}
									<span class="badge">{$newLang|count}</span> {l s='expressions' d='Admin.International.Feature'} <span class="label label-danger">{$missing_langs_module}</span>
								</h3>
								<div name="{$type}_div" id="{$theme_name}_{$module_name}_{$template_name|replace:'.':'_'}" style="display:{if $missing_langs_module}block{else}none{/if}">
									<table class="table">
										{foreach $newLang as $key => $value}
											<tr>
