...
export default class OrderPricesRefresher {
   ...
   * and if so and if the price of the 2 inventaries doesn't match will return either
   * 'invoice' if the 2 inventaries are in 2 different invoices or 'product' if the 2 inventaries
   * are in the same invoice (or no invoice yet). Only inventaries that have different customizations
   * can be twice in a same invoice.
   * Will return null if no matching inventaries are found.
   */
