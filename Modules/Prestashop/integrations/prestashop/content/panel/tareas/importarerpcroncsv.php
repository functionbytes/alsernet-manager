...
function ProcesarProducto($data, $fila, $tipo){
        ...
        if ($idproductattributeps!=""){
           ...
           Db::getInstance()->Execute("UPDATE aalv_combinaciones_import set unidades_oferta=".$data["unidades_oferta"].",etiqueta='".$data["etiqueta"]."',estado_gestion=".$data["estado"].",es_segunda_mano=".$data["es_segunda_mano"].",externo_disponibilidad=".$data["externo_disponibilidad"].",codigo_proveedor='".$data["codigo_proveedor"]."',precio_costo_proveedor=".$data["precio_costo_proveedor"].",tarifa_proveedor=".$tarifaproveedor.",es_arma=".$esarma.",es_arma_fogueo=".$esarmafogueo.",es_cartucho=".$escartucho.",categoria=".$categoria.",familia=".$familia.",subfamilia=".$subfamilia.",grupo=".$grupo." WHERE id_product_attribute=".$idproductattributeps);
        ...
        if ($idprodpssinatributo!=""){
            ...
            Db::getInstance()->Execute("UPDATE aalv_combinacionunica_import set unidades_oferta=".$data["unidades_oferta"].",etiqueta='".$data["etiqueta"]."',estado_gestion=".$data["estado"].",es_segunda_mano=".$data["es_segunda_mano"].",externo_disponibilidad=".$data["externo_disponibilidad"].",codigo_proveedor='".$data["codigo_proveedor"]."',precio_costo_proveedor=".$data["precio_costo_proveedor"].",tarifa_proveedor=".$tarifaproveedor.",es_arma=".$esarma.",es_arma_fogueo=".$esarmafogueo.",es_cartucho=".$escartucho.",categoria=".$categoria.",familia=".$familia.",subfamilia=".$subfamilia.",grupo=".$grupo." WHERE id_product=".$idprodpssinatributo);
...
function ProcesarPrincipal($idmodelo){
    ...
    if ($idproducto!=""){
        ...
        $datos = "".Db::getInstance()->getValue("SELECT data  FROM aalv_integracion_cambios WHERE tabla LIKE '%perfiles_nav%' AND data LIKE '%".$idmodelo."%' and data like '%true%'");
...
function recuperarcsv($filename){
  $filas=[];
  if (($gestor = fopen(__DIR__."/".$filename, "r")) !== FALSE) {
    $cabecera = fgetcsv($gestor, 0, ";");
    $numero = count($cabecera);
    while (($datos = fgetcsv($gestor, 0, ";")) !== FALSE) {
        $fila=[];
        for ($c=0; $c < $numero; $c++) {
          $fila[$cabecera[$c]]=$datos[$c];
        }
        $filas[]=$fila;
    }
    fclose($gestor);
...
if (($activo==1) && ($activofecha=="")){
    ...
    $resultado = recuperarcsv("transaccion.csv");
    ...
    foreach($resultado as $valores){
        ...
        if ($existe==""){
            ...
            echo "llega....".$transaccion.".....<br/>";

            Db::getInstance()->Execute("INSERT INTO aalv_integracion_transacciones(id, url_transaccion, fecha_creacion, primerid, total_cambios, fecha_confirmacion) VALUES ('".$valores["transaccion"]."','".$valores["url_transaccion"]."','".str_replace(".0","",$valores["fecha_creacion"])."',".$valores["primer_idcambiotabla"].",".$valores["total_cambios"].",'0000-00-00 00:00:00')");
            ...
            if ($count>0){
                ...
                foreach($objitem["results"] as $valoresitem){
                    ...
                    Db::getInstance()->Execute("INSERT INTO aalv_integracion_cambios(tabla, fila, data, tipo, transaccion, fecha_confirmacion) VALUES ('".$valoresitem["tabla"]."',".$valoresitem["fila"].",'".json_encode($valoresitem["data"])."','".$valoresitem["tipo"]."','".$transaccion."','0000-00-00 00:00:00')");
                    ...
                    try{
                        ...
                        sendmail("error ".$e->getMessage(). "<br/>integracion ".$valoresitem["tabla"]." ".$valoresitem["fila"]." ".$valoresitem["tipo"]." ".$transaccion);
                ...
                if ($count==$contarexito){
                    ...
                    //dump($objconfirm);
                        ...
                        Db::getInstance()->Execute("Update aalv_integracion_cambios set fecha_confirmacion='".str_replace(".0","",$valores["fecha_confirmacion"])."' where transaccion='".$transaccion."'");
                        Db::getInstance()->Execute("Update aalv_integracion_transacciones set fecha_confirmacion='".str_replace(".0","",$valores["fecha_confirmacion"])."' where id='".$transaccion."'");
            ...
            echo "<br/>Datos totales:". $count. " datos con exito:" . $contarexito. "<br/>";
        ...
        else
        {
            ...
            try{
                ...
                $count = $objitem["count"];
                echo "numfilas ".$count."<br/>";
            } catch (Exception $e) {
                $count = 0;
                echo "numfilas2 ".$count."<br/>";
            }
            ...
            if ($count>0){
                ...
                if ($count==$contarexito){
                    ...
                    echo "Exitos ".$contarexito. "<br/>";
                    echo "Transaccion ".$transaccion. " NO CONFIRMADA<br/>";

                    sendmailhora("Transaccion ".$transaccion. " NO CONFIRMADA<br/>");
...
echo "Proceso terminado";
    ...
    sendmail("error ".$e->getMessage()."<br/>Sale de la tarea");
