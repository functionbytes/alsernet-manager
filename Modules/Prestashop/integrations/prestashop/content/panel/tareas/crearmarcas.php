...
try {
    ...
    $dsn2 = "mysql:host=195.55.36.104;dbname=alvarezseo";
    $dbh2 = new PDO($dsn2, 'tiendalvad', 'Nov.299909');
...
$rows = getdatarows($dbh,"SELECT id, nombre, safe_name, logo, posicion, deporte, tiene_productos_activos FROM marcas order by 1");
foreach($rows as $row){
    CrearMarca($row, $dbh);
}
...
//SELECT id_marca, deporte FROM marcas_deporte


/*
Db::getInstance()->Execute("truncate table aalv_manufacturer_deporte");
Db::getInstance()->Execute("truncate table aalv_manufacturer_deporte_lang");
