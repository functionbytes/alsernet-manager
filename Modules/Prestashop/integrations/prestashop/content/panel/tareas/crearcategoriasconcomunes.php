...
function Verificarcategoria($row, $dbh){

    $id_nav = $row["id_nav"];
    $id_cat  = $row["id_cat"];
    $existeidnav = "".getfieldvalue($dbh,"SELECT id FROM navegacion WHERE id=".$id_nav);
...
/*
$cateventos=3551;
$childreneventos = Category::getChildren($cateventos, 1, false, false);
foreach($childreneventos as $row){
    $category = new Category($row["id_category"]);
    $category->delete();
    echo "Pasa ".$row["id_category"];
}
*/
