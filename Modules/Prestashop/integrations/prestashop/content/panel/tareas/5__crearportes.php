...
function CrearPortesTipo($portestipo, $dbh){
        Db::getInstance()->Execute("INSERT INTO aalv_portes_tipo(tipo_es,plazo_es,tipo_en,plazo_en) VALUES ('".$portestipo["tipo_es"]."','".$portestipo["plazo_es"]."','".$portestipo["tipo_en"]."','".$portestipo["plazo_en"]."')");
}



function CrearPortes($portes, $dbh){
        ...
        Db::getInstance()->Execute("INSERT INTO aalv_portes(id_porte, codigo, importe, acumulable, tipo) VALUES (".$idorigen.",'".$portes["codigo"]."',".$portes["importe"].",".$portes["acumulable"].",".$portes["tipo"].")");
}
...
function CrearPortesProducto($portesproducto, $dbh){


        $referencia=$portesproducto["referencia"]; 
        ...
        if ($idproduct!=""){
            ...
            $idpaisps=getpaisps($portesproducto["id_pais"]);

             Db::getInstance()->Execute("INSERT INTO aalv_portes_producto(id_origen,referencia,codigo,id_pais_origen,id_pais,id_product,id_product_attribute) VALUES (".$portesproducto["id"].",'".$referencia."','".$portesproducto["codigo"]."',".$portesproducto["id_pais"].",".$idpaisps.",".$idproduct.",".$idproductattribute.")");

        }
        else{
            $d = new DateTime();
            $stdout = fopen(dirname(__FILE__).'/porteserrores.txt', 'a');
            fwrite($stdout, $d->format("Y-m-d\TH:i:sP")." --- error No existe la referencia ".$referencia);    
            fwrite($stdout, "\n"); 
            fwrite($stdout, " --- datos ".$portesproducto[0]);    
            fwrite($stdout, "\n"); 
            fclose($stdout);  
...
Db::getInstance()->Execute("truncate table aalv_portes");
Db::getInstance()->Execute("truncate table aalv_portes_tipo");
Db::getInstance()->Execute("truncate table aalv_portes_producto");




$rows = getdatarows($dbh, "SELECT * FROM portes_tipo order by id asc");
foreach($rows as $row){
    CrearPortesTipo($row, $dbh);
}  


$rows = getdatarows($dbh, "SELECT * FROM portes");
foreach($rows as $row){
    CrearPortes($row, $dbh);
}  
  ...
  $listaespeciales="J,K,G,E2,D1,H,I,N,Y,X,M,P,C,O,CM2,V,D,F,J1,J2,DD1,S,G1,O1,Z1,Q1,E1,REINO UNIDO SILLA EXPRESS,Z2,J1,LANCHAS,GUADARNES,SIN PORTES,REINO UNIDO CAÑAS,ARMERO PORTUGAL (1),ARMERO BALEARES,ARMERO ESPAÑA BALEARES,ARMERO ESPAÑA BALEARES (2),ARMERO PORTUGAL  (2)";  


  $codigos=explode(",", $listaespeciales);
  foreach($codigos as $codigo){

      $rows = getdatarows($dbh, "select * from portes_producto where codigo ='".$codigo."'");
        foreach($rows as $row){
            CrearPortesProducto($row, $dbh);
        }  
