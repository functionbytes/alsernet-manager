...
try{
    ...
    if ($tipo<=2){
        ...
        $specificprice="".Db::getInstance()->getValue("SELECT id_specific_price FROM aalv_specific_price_import WHERE id_tarifa_cabecera=".$id_tarifa_cabecera);
        if ($specificprice!=""){
...
function addlogtarifas($message){
    $d = new DateTime();
    $stdout = fopen(dirname(__FILE__).'/logtarifasdef.txt', 'a');
...
$sql = "SELECT * FROM aalv_tarifa_cabecera_import where ((finicio<=now() and ffin>=now()) or (finicio>=now()) or (finicio<=now() and ffin='0000-00-00 00:00:00')) and id_tarifa_cabecera not in (select id_tarifa_cabecera from aalv_specific_price_import)  ";
...
foreach($rows as $row){
    ...
    $sql = "SELECT id FROM `aalv_integracion_cambios` WHERE `tabla` = 'v_sinc_tarifa_cabecera' and fila=".$idtarifacabecera." ORDER BY `id` desc";


    $idlinea = "".Db::getInstance()->getValue($sql);

    if ($idlinea!=""){
        addlogtarifas($idlinea);
        $valoresitem = Db::getInstance()->getRow("select * from aalv_integracion_cambios where id=".$idlinea);
