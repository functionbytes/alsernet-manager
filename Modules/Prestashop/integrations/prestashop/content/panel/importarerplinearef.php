...
$referencias = "'WP307406-1'";

$refe = explode(',', $referencias);
...
foreach ($refe as $ref) {
    ...
    $productoerp = '';
    $productoatributops = ''.Db::getInstance()->getValue("SELECT id_product_attribute FROM aalv_product_attribute WHERE reference='".str_replace("'", '', $ref)."'");
    if ($productoatributops == '') {
        $productops = ''.Db::getInstance()->getValue("SELECT id_product FROM aalv_product WHERE reference='".str_replace("'", '', $ref)."'");
        if ($productops != '') {
            // buscar en combinacionunica
            $productoerp = ''.Db::getInstance()->getValue('SELECT id_origen FROM aalv_combinacionunica_import where id_product='.$productops);
        }

    } else {
        // buscar en combinaciones
        $productoerp = ''.Db::getInstance()->getValue('SELECT id_origen FROM aalv_combinaciones_import where id_product_attribute ='.$productoatributops);
