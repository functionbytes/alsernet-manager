...
// $columna = Db::getInstance()->ExecuteS("select id_product,id_articulo from aalv_combinacionunica_import aci where id_product not in (
//     16122,16123,16265,16266,20825,20826
// ) order by id_product ASC");
// $nn = 0;
// $ids = [];
// $dbcon = connectBD();
// foreach ($columna as $value) {
//     $columna2 = Db::getInstance()->ExecuteS("select `to`, `from`, id_product from aalv_specific_price asp  WHERE id_product = ".$value['id_product']." and id_country = 0 order by id_specific_price DESC limit 1");
...
//                                                         WHERE
//                                                             ap.id_product = ".$columna2[0]['id_product']);

//             if($columna22[0]['active'] == 1 && $columna22[0]['active_shop'] == 1){

//                 if (!in_array($columna2[0]['id_product'], $ids)) {
//                     $id_modelo = Db::getInstance()->getValue("select id_modelo from aalv_product_import api where id_product =".$columna2[0]['id_product']);
...
//                                                                 WHERE
//                                                                     acp.id_product = ".$columna2[0]['id_product']."
//                                                                     and acp.id_category IN (3,4,5,6,7,8,9,10,11)
//                                                                     and acl.id_lang = 1");

//                     $ids[] = $columna2[0]['id_product'];
//                     echo "id_product => ".$columna2[0]['id_product']."<br>";
//                     echo $id_modelo."<br>";
//                     foreach ($category as $val) {
//                         echo "deporte => ".$val['name']."<br>";
//                     }
//                     echo "id_articulo => ".$value['id_articulo']."<br>";
//                     echo "reference => ".$columna22[0]['reference']."<br>";
//                     $nn++;
//                     echo "NN => ".$nn."<br><hr>";
...
//                     // peticionget("https://www.a-alvarez.com/?fc=module&module=pagecache&controller=clearcache&token=ApbUf8KuFaGPBhAk&product=".$columna2[0]['id_product']);
...
// $columna = Db::getInstance()->ExecuteS("SELECT id_product FROM aalv_feature_product afp WHERE id_feature = 20 AND id_feature_value = 200227 and id_product > 64636");


// foreach ($columna as $value) {
//     $product = new Product($value['id_product']);
//             // $product->addFeaturesToDB($feature, $valor); 7580
//             $product->update();
...
// $columna = Db::getInstance()->ExecuteS("select id_product_attribute, id_articulo from aalv_combinaciones_import aci order by id_product_attribute DESC");
...
$columna = Db::getInstance()->ExecuteS("SELECT
pri.id_product_attribute,
pri.id_product
FROM
aalv_product_attribute pri
LEFT JOIN aalv_specific_price apa on apa.id_product_attribute = pri.id_product_attribute
left join aalv_product ap on ap.id_product = pri.id_product
left join aalv_tot_switch_attribute_disabled atsad on atsad.id_product_attribute = pri.id_product_attribute
WHERE
apa.id_product_attribute  IS NULL
and pri.id_product_attribute != 0
and ap.active = TRUE
and atsad.id_product_attribute IS NULL
and pri.id_product not in (56764,55050,57934,57946,57950,57952,57954,61373,61569,52930,52938,61789,61863,62667,63543,64981,64826,63957)
order by  pri.id_product_attribute DESC");

foreach ($columna as $value) {
    $datos = Db::getInstance()->ExecuteS("select * from aalv_specific_price asp where id_product = ".$value['id_product']." and id_product_attribute = 0");
    if(count($datos) < 2){
        echo "id_product_attribute => ".$value['id_product_attribute']."\n";
        echo "id_product => ".$value['id_product']."\n\n--------------\n";
    }
