...
class TreeBuilder
{
    ...
    public function makeTranslationArray(AbstractProvider $provider, $search = null)
    {
        ...
        foreach ($defaultCatalogue->all() as $domain => $messages) {
            ...
            $translations[$domain]['__metadata'] = ['missing_langs' => $missingTranslations];
        }
        ...
        foreach ($tree as $k1 => $t1) {
            ...
            if (is_array($t1) && '__' !== substr($k1, 0, 2)) {
                ...
                if (array_key_exists('__messages', $t1)) {
                    ...
                    $rootTree['tree']['total_langs'] += $nbMessage;

                    if (array_key_exists('__metadata', $t1) && array_key_exists('missing_langs', $t1['__metadata'])) {
                        $cleanTree[$index1]['total_missing_langs'] += (int) $t1['__metadata']['missing_langs'];
                        $rootTree['tree']['total_missing_langs'] += (int) $t1['__metadata']['missing_langs'];
                    }
                }
                ...
                foreach ($t1 as $k2 => $t2) {
                    ...
                    if (is_array($t2) && '__' !== substr($k2, 0, 2)) {
                        ...
                        if (array_key_exists('__messages', $t2)) {
                            ...
                            if (array_key_exists('__metadata', $t2) && array_key_exists('missing_langs', $t2['__metadata'])) {
                                $cleanTree[$index1]['children'][$index2]['total_missing_langs'] += (int) $t2['__metadata']['missing_langs'];
                                $cleanTree[$index1]['total_missing_langs'] += (int) $t2['__metadata']['missing_langs'];
                                $rootTree['tree']['total_missing_langs'] += (int) $t2['__metadata']['missing_langs'];
                            }
                        }
                        ...
                        foreach ($t2 as $k3 => $t3) {
                            if (is_array($t3) && '__' !== substr($k3, 0, 2)) {
                                ...
                                if (array_key_exists('__metadata', $t3) && array_key_exists('missing_langs', $t3['__metadata'])) {
                                    $cleanTree[$index1]['children'][$index2]['children'][$index3]['total_missing_langs'] += (int) $t3['__metadata']['missing_langs'];
                                    $cleanTree[$index1]['children'][$index2]['total_missing_langs'] += (int) $t3['__metadata']['missing_langs'];
                                    $cleanTree[$index1]['total_missing_langs'] += (int) $t3['__metadata']['missing_langs'];
                                    $rootTree['tree']['total_missing_langs'] += (int) $t3['__metadata']['missing_langs'];
                                }
    ...
    private function addTreeInfo(Router $router, &$tree, $index, $name, $fullName, $theme = false, $search = null, $module = false)
    {
        if (!isset($tree[$index])) {
            ...
            $tree[$index]['total_langs'] = 0;
            $tree[$index]['total_missing_langs'] = 0;
            $tree[$index]['children'] = [];
        }
