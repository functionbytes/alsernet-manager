...
abstract class TaxManagerModuleCore extends Module
{
    ...
    public function hookTaxManager($args)
    {
        ...
        if (! isset($this->tax_manager_class) || ! file_exists($class_file)) {
            exit($this->trans('Incorrect Tax Manager class [%s]', [$this->tax_manager_class], 'Admin.International.Notification'));
        }

        require_once $class_file;

        if (! class_exists($this->tax_manager_class)) {
            exit($this->trans('Tax Manager class not found [%s]', [$this->tax_manager_class], 'Admin.International.Notification'));
        }
