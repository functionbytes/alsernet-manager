...
class Product extends ProductCore
{
    ...
    public function deleteAlsernet($products = [])
    {
        $return = false;
        if (! empty($products)) {
    ...
    public static function getAttributesColorList(array $products, $have_stock = true)
    {
        if (Module::isEnabled('totswitchattribute')) {
            ...
            if (Configuration::get('TOT_SWITCH_MODE') == TotSwitchAttribute::MODE_HIDE) {
    ...
    public function getAttributesGroups($id_lang, $id_product_attribute = null)
    {
        if (Module::isEnabled('totswitchattribute')) {
            ...
            if (version_compare(_PS_VERSION_, '1.7', '<') && Configuration::get('TOT_SWITCH_MODE') == TotSwitchAttribute::MODE_HIDE) {
    ...
    public function needDefaultAttribute($array, $defaultField, $defaultValue, $groupAttributeField)
    {
        ...
        if (
            ...
            && Configuration::get('TOT_SWITCH_MODE') != TotSwitchAttribute::MODE_HIDE
        ) {
            $switch_attr_groups = [];
            foreach ($array as $key => $attribute) {
            ...
            foreach ($switch_attr_groups as $key => $attribute) {
                if (
                    array_unique($attribute['enabled_attribute']) != ['0']
    ...
    public static function getVisibilidad($id_product, $id_product_attribute = false, $debug = false)
    {
        ...
        if ($product_import[0]['activo'] == 0) {
            if ($debug) {
                var_dump('No visible en gestión');
            }
    ...
    public function getCategoryParent($idCategory = null, $idLang = 1)
    {
        $sql = '
    ...
    public function getTopParentCategory($idCategory, $idLang = null, $idShop = null)
    {
    ...
    public static function hasFeature($id_product, $feature_value)
    {
        $hasFeature = false;
    ...
    public function getExtraByProduct($id_product, $id_cart = false)
    {
        ...
        $this->smarty->assign(['edit_link' => $edit_url]);
