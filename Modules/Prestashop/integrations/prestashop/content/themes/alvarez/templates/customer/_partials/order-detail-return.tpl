...
{block name='order_products_table'}
  <form id="order-return-form" class="js-order-return-form" action="{$urls.pages.order_follow}" method="post">
      ...
      <table id="order-products" class="table table-bordered return">
        <thead class="thead-default">
          <tr>
            ...
            <th>{l s='Product' d='Shop.Customer.Orders'}</th>
            <th>{l s='Quantity' d='Shop.Customer.Orders'}</th>
            <th>{l s='Returned' d='Shop.Customer.Orders'}</th>
            <th>{l s='Unit price' d='Shop.Customer.Orders'}</th>
            <th>{l s='Total price' d='Shop.Customer.Orders'}</th>
          </tr>
        </thead>
          ...
          <tr>
            ...
            <td>
              ...
              {if $product.product_reference}
                {l s='Reference' d='Shop.Customer.Orders'}: {$product.product_reference}<br/>
              {/if}
              {if $product.customizations}
                {foreach from=$product.customizations item="customization"}
                  <div class="customization">
                    <a href="#" data-toggle="modal" data-target="#product-customizations-modal-{$customization.id_customization}">{l s='Product customization' d='Shop.Customer.Orders'}</a>
                  </div>
                      ...
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            ...
                            <h4 class="modal-title">{l s='Product customization' d='Shop.Customer.Orders'}</h4>
                          </div>
            ...
            <div class="content">
              <div class="row">
                <div class="col-sm-5 desc">
                  ...
                  {if $product.product_reference}
                    <div class="ref">{l s='Reference' d='Shop.Customer.Orders'}: {$product.product_reference}</div>
                  {/if}
                  {if $product.customizations}
                    {foreach $product.customizations as $customization}
                      <div class="customization">
                ...
                <div class="col-sm-7 qty">
                  <div class="row">
                    ...
                    <div class="col-xs-4">
                      {if $product.customizations}
                        {foreach $product.customizations as $customization}
                          <div class="q">{l s='Quantity' d='Shop.Customer.Orders'}: {$customization.quantity}</div>
                          <div class="s" id="_mobile_return_qty_{$product.id_order_detail}_{$customization.id_customization}"></div>
                        {/foreach}
                      {else}
                        <div class="q">{l s='Quantity' d='Shop.Customer.Orders'}: {$product.quantity}</div>
                        {if $product.quantity > $product.qty_returned}
                          <div class="s" id="_mobile_return_qty_{$product.id_order_detail}"></div>
                        {/if}
                      {/if}
                      {if $product.qty_returned > 0}
                        <div>{l s='Returned' d='Shop.Customer.Orders'}: {$product.qty_returned}</div>
                      {/if}
                    </div>
    ...
    <div class="box">
      <header>
        <h3>{l s='Merchandise return' d='Shop.Customer.Orders'}</h3>
        <p>{l s='If you wish to return one or more inventaries, please mark the corresponding boxes and provide an explanation for the return. When complete, click the button below.' d='Shop.Customer.Orders'}</p>
      </header>
      ...
      <footer class="form-footer">
        ...
        <button class="form-control-submit btn btn-primary" type="submit" name="submitReturnMerchandise">
          {l s='Request a return' d='Shop.Customer.Orders'}
        </button>
      </footer>
    </div>
