    ...
    {if $field.name != 'newsletter'}
        ...
        <div class="form-group {if !empty($field.errors)}has-error{/if}">
            <div class="row">
                    ...
                    {if $field.name != 'vat_number' }
                        <label class="col-md-12 form-control-label{if $field.required} required{/if}" for="field-{$field.name}">
                            ...
                            {if $field.name == 'birthday'}
                                <span class="opcional">({l s='Optional' d='Shop.Forms.Labels'})</span>
                            {/if}
                        </label>
                    {elseif $need_invoice_option && $field.name == 'vat_number'}
                            <label class="col-md-12 form-control-label" >
                                {$field.label}
                            </label>
                    {elseif !$need_invoice_option}
                        <label class="col-md-12 form-control-label{if $field.required} required{/if}" for="field-{$field.name}">
                            {$field.label}
                            {if $field.name == 'birthday'}
                                <span class="opcional">({l s='Optional' d='Shop.Forms.Labels'})</span>
                ...
                <div class="{if $field.type !== 'checkbox'}col-md-12{else}{if $page.page_name != 'identity'}col-md-12{else}col-md-12{/if}{/if} {if $field.name == 'birthday'}cumple-col{/if} {if ($field.type === 'radio-buttons')} form-control-valign{/if}">
                ...
                <span class="custom-radio">
                  ...
                  {l s='Show' d='Shop.Theme.Actions'}
                    ...
                    {else}
                        ...
                        {block name='form_field_item_sports'}
                        {/block}

                        {block name='form_field_item_other'}


                            {if $need_invoice_option && $field.name == 'vat_number'}
                                <input
                                        class="form-control"
                                        name="{$field.name}"
                                        type="{$field.type}"
                                ...
                                <input
                                        id="field-{$field.name}"
                                        class="form-control {if $field.name == 'birthday'}cumple{/if}"
                                        name="{$field.name}"
                                        type="{$field.type}"
                                        value="{$field.value}"
                                        {if $field.autocomplete}autocomplete="{$field.autocomplete}"{/if}
                            ...
                            {if $field.name == 'birthday'}
                                <span class="texto-cheque-regalo">{l s="Te enviaremos un cheque regalo por tu cumpleaños" d="Shop.Forms.Labels"}</span>
                            {/if}
                            {if isset($field.availableValues.comment)}
                                <span class="form-control-comment">
                                    {$field.availableValues.comment}
