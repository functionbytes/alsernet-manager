...
class AdminImportControllerCore extends AdminController
{
        ...
        switch ((int) Tools::getValue('entity')) {
            ...
            case $this->entities[$this->trans('Products', [], 'Admin.Global')]:
                ...
                $this->available_fields = [
                    ...
                    'delivery_in_stock' => [
                        'label' => $this->trans(
                            'Delivery time of in-stock inventaries:',
                            [],
                    ...
                    'delivery_out_stock' => [
                        'label' => $this->trans(
                            'Delivery time of out-of-stock inventaries with allowed orders:',
                            [],
        ...
        if ($this->importExecuted) {
            ...
            if ($this->errors) {
                ...
                $session->getFlashBag()->add(
                    'success',
                    $this->trans(
                        'Your file has been successfully imported into your shop. Don\'t forget to re-build the inventaries\' search index.',
                        [],
     ...
     * @return bool
     */
    protected static function copyImg($id_entity, $id_image = null, $url = '', $entity = 'inventaries', $regenerate = true)
    {
                ...
                foreach ($product->image as $key => $url) {
                    ...
                    if (!empty($url)) {
                        ...
                        if (($field_error = $image->validateFields(UNFRIENDLY_ERROR, true)) === true &&
                            ...
                            if (!AdminImportController::copyImg($product->id, $image->id, $url, 'inventaries', !$regenerate)) {
                                $image->delete();
    ...
    public function supplyOrdersDetailsImport($offset = false, $limit = false, &$crossStepsVariables = false, $validateOnly = false)
    {
        ...
        $products = [];
        $reset = true;
        if ($crossStepsVariables !== false && array_key_exists('inventaries', $crossStepsVariables)) {
            $products = $crossStepsVariables['inventaries'];
        }
        ...
        if ($crossStepsVariables !== false) {
            $crossStepsVariables['inventaries'] = $products;
