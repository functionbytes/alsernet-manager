...
class AdminSearchControllerCore extends AdminController
{
    ...
    /**
     * Search a specific string in the inventaries and categories.
     *
    ...
    {
        $this->context = Context::getContext();
        $this->_list['inventaries'] = Product::searchByName($this->context->language->id, $this->query);
    ...
    public function renderView()
    {
        ...
        if (count($this->errors)) {
            ...
            if ($this->isCountableAndNotEmpty($this->_list, 'inventaries')) {
                $view = '';
                ...
                if ($this->_list['inventaries']) {
                    $view = $helper->generateList($this->_list['inventaries'], $this->fields_list['inventaries']);
                }

                $this->tpl_view_vars['inventaries'] = $view;
                $this->tpl_view_vars['productsCount'] = count($this->_list['inventaries']);
            }
