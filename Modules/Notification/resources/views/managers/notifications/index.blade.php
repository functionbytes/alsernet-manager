@extends('resources.views.layouts.managers')

@section('content')

    @include('resources.views.managers.components.card', ['title' => 'Notificaciones'])
        ...
        <div class="card-body">
            ...
            <ul class="nav nav-pills mb-3" role="tablist">
                <li class="nav-item">
                    <a class="nav-link {{ $filter === 'all' ? 'active' : '' }}"
                       href="{{ route('manager.notifications.index', ['filter' => 'all']) }}">
                ...
                <li class="nav-item">
                    <a class="nav-link {{ $filter === 'unread' ? 'active' : '' }}"
                       href="{{ route('manager.notifications.index', ['filter' => 'unread']) }}">
                ...
                <li class="nav-item">
                    <a class="nav-link {{ $filter === 'read' ? 'active' : '' }}"
                       href="{{ route('manager.notifications.index', ['filter' => 'read']) }}">
            ...
            @if($unreadCount > 0)
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <form action="{{ route('manager.user-notifications.mark-all-read') }}" method="POST"
                    ...
                    @foreach($notifications as $notification)
                        ...
                        <div class="list-group-item list-group-item-action {{ $isUnread ? 'bg-light-subtle' : '' }}">
                            <div class="d-flex gap-3 align-items-start">
                                ...
                                <span
                                    class="bg-{{ $colorClass }}-subtle rounded-circle d-flex align-items-center justify-content-center text-{{ $colorClass }}"
                                ...
                                <div class="flex-grow-1" style="min-width: 0;">
                                        ...
                                        <h6 class="mb-0 fw-semibold">
                                            ...
                                            @if($isUnread)
                                                <span class="badge bg-primary rounded-pill ms-2"
                                                      style="font-size: 9px;">Nuevo</span>
                                            @endif
                                        </h6>
                                        <small
                                    ...
                                    <div class="d-flex gap-2">
                                        ...
                                        @if($isUnread)
                                            <form
                                                action="{{ route('manager.user-notifications.mark-as-read', $notification->id) }}"
                                        ...
                                        <form
                                            action="{{ route('manager.user-notifications.destroy', $notification->id) }}"
                                            method="POST" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
