 ...
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation search($keyword)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereCreatedAt($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereData($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereId($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereLastError($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereMailListId($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereName($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereSegmentId($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereStatus($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereTimeZone($value)
 * @method static \Illuminate\Database\Eloquent\Builder<static>|Automation whereUid($value)
...
class Automation extends Model
{
        ...
        foreach ($entries as $entry) {
            ...
            if ($action->getType() == 'ElementAction') {
                if ($action->getOption('init') == 'true') { // @IMPORTANT VERY IMPORTANT! Use "true", it is because even "false" == true!!!!
                    if (! $this->emails()->where('uid', $action->getOption('mail_uid'))->exists()) {
    ...
    public function getAbandonedCartEmail()
    {
        ...
        return \App\Models\Email::findByUid($this->getElements()[1]->options->mail_uid);
    }
    ...
    public function deleteAndCleanup()
    {
        ...
        $this->getActions(function ($action) {
            ...
            if (isset($options['mail_uid'])) {
                $originalEmail = Email::findByUid($options['mail_uid']);
    ...
    public function duplicate()
    {
        ...
        $newAutomation->getActions(function ($action) use ($newAutomation) {
            ...
            if (isset($options['mail_uid'])) {
                ...
                // $action->setOption('mail_uid', $newEmail->uid);
                $newAutomation->data = str_replace($options['mail_uid'], $newEmail->uid, $newAutomation->data);
                $newAutomation->save();
            }
        });
