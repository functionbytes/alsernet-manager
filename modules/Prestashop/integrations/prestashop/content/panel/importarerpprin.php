...
$sql = Db::getInstance()->ExecuteS("SELECT data FROM aalv_integracion_cambios WHERE tabla LIKE '%perfiles_nav%'");
$array_modelo = [];
foreach ($sql as $value) {
    $data = json_decode($value['data'], true);
    if (! is_null($data)) {
        $array_modelo[] = $data['id_modelo'];
    }
}
$array_modelo = array_values(array_unique($array_modelo));

// $rowsp =  Db::getInstance()->ExecuteS("SELECT id_product, id_modelo FROM aalv_product_import WHERE id_product>=55053 and id_product in (SELECT id_product  FROM aalv_product WHERE id_category_default = 2)");
exit();
foreach ($array_modelo as $prod) {
    // $producto = $prod["id_product"];
    // $modelo = $prod["id_modelo"];
    ProcesarPrincipal($prod);
}
