...
<li class="tree-folder">
	<span class="tree-folder-name{if isset($node['disabled']) && $node['disabled'] == true} tree-folder-name-disable{/if}">
		...
		<label class="tree-toggler">{if isset($node['name'])}{if $node['id_category']}{$node['id_category']}-{/if}{$node['name']|escape:'html':'UTF-8'}{/if}{if isset($node['selected_childs']) && (int)$node['selected_childs'] > 0} {l s='(%s selected)' sprintf=[$node['selected_childs']]}{/if}</label>
	</span>
	<ul class="tree">
