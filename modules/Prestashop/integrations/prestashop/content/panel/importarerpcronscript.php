...
function ProcesarProducto($data, $fila, $tipo)
{
    ...
    try {
        ...
        if ($idprodps == '') {
            ...
            sendmail2('Viene producto '.$idproducterp.' antes de crear el modelo '.$idmodelo.' - NUEVO CAMBIO');
        ...
        echo 'aca => id_pruduct: '.$idprodps.' - id_product_attribute: '.$idproductattributeps.' - id_product_combinacionunica: '.$idprodpssinatributo."\n";
        ...
        if ($tipo <= 2) {
            if ($tipo == 1) {
                ...
                if ($idprodps != '') {
                    ...
                    if ($existencombinaciones != 0) {
                        ...
                        /* nuevo */
                        if ($traspasarstockprecios) {
                            Cambiarproducto($idprodps, $idProductAttribute, $idprodpsanterior, $idpscombanterior);
                        }
                    } else {
                        ...
                        if ($existeunica == '') {
                            ...
                            */
                            ...
                            /* nuevo */
                            if ($traspasarstockprecios) {
                                Cambiarproducto($idprodps, '0', $idprodpsanterior, $idpscombanterior);
                            }
                            // arama lo que venga
                    ...
                    // $product->active = Product::getVisibilidad($idprodps, $idProductAttribute);
                    $product->update();
                    // ControlStock($idprodps);
                } else {
                    // producto erp sin modelo
                ...
                if ($idprodpssinatributo != '') {
                    ...
                    */
                    ...
                    // $product->active = Product::getVisibilidad($idprodpssinatributo, false);
                    $product->update();
                    // ControlStock($proddelattr);
                }
                ...
                if ($idproductattributeps != '') {
                    ...
                    if ($existe != '') {
                        ...
                        if ($proddelattr != '') {
                            ...
                            // $product->active = Product::getVisibilidad($proddelattr, $idproductattributeps);
                            $product->update();
                            // ControlStock($proddelattr);
                        }
                    }
            ...
            if (strpos($data['etiqueta'], 'VENTA_ESP') !== false) {
                if (! validarFeature(VENTA_ESP_FEATURE, $idprodps)) {
            ...
            if ($data['es_segunda_mano'] != 0) {
                if (! validarFeature(ES_SEGUNDA_MANO_FEATURE, $idprodps)) {
                ...
                if (! validarFeature(ES_SEGUNDA_MANO_FINANCIACION_FEATURE, $idprodps)) {
...
function Cambiarproducto($idprodps, $idProductAttribute, $idprodpsanterior, $idpscombanterior)
{
    ...
    procesarcombinaciones($idprodpsanterior);
}
    ...
    if ($tipo <= 2) {
        ...
        if ($idprodps != '') {
            ...
            */
            ...
            // $product->active = Product::getVisibilidad($product->id, false);
            $product->update();
            ...
            // Db::getInstance()->Execute("Update aalv_product set visibility='".$visibilidad."' where id_product=".$product->id);
            // Db::getInstance()->Execute("Update aalv_product_shop set visibility='".$visibilidad."' where id_product=".$product->id);

            // ControlStock($product->id);
            // crearimagenmodelo($product->id, $imagesurls);

            // Realizamos traducción
            /*
            ...
            $retval=null;
            foreach ($idiomas as $idioma) {
                addlog("Traducimos el producto: ".$product->id." al ".$idioma['iso_code']);
                exec('/usr/bin/php /var/www/clients/client1/web1/web/bin/console dgcontenttranslation:translate --from_lang=es --dest_lang='.$idioma['iso_code'].' --tables="product_lang" --overwrite=on --range='.$product->id.':'.$product->id, $output, $retval);
                addlog($retval);
                addlog("Fin de la traducción del producto: ".$product->id." al ".$idioma['iso_code']);
            }
            */
...
function ProcesarStockCentralWeb($data, $fila, $tipo)
{
    ...
    if ($tipo <= 2) {
        ...
        if ($idproductattribute != '') {
            ...
            $externo_disponibilidad = $result[0]['externo_disponibilidad'];
        } else {
        ...
        if ($idproduct != '') {
            ...
            // $es_lote = "" . Db::getInstance()->getValue("SELECT id_wk_bundle_product FROM aalv_wk_bundle_product WHERE id_ps_product=" . $idproduct);
            // if ($es_lote != '' || $externo_disponibilidad) {
            //     $unidades = 999999;
            // }
            ...
            // if($unidades_pocomaco < 0){
            //     $unidades_pocomaco = 0;
            // }

            // $unidades = Product::controlStock((int)$idproduct, (int)$idproductattribute, $unidades);
            ...
            if ($existe == '') {
                Db::getInstance()->Execute('INSERT INTO aalv_repositorio_stock(id_product, id_product_attribute, quantity, quantity_pocomaco) VALUES ('.$idproduct.','.$idproductattribute.','.$unidades.', '.$unidades_pocomaco.')');
            } else {
                Db::getInstance()->Execute('UPDATE aalv_repositorio_stock SET quantity='.$unidades.', quantity_pocomaco = '.$unidades_pocomaco.' WHERE id='.$existe);
            }

            procesarcombinaciones($idproduct);

            // $product = new Product($idproduct);
            // $product->active = Product::getVisibilidad($idproduct, $idproductattribute);
            // $product->update();
            // dump($data["idarticulo"]);
            Product::alsernetNewVisibilidad($data['idarticulo']);
            // dump('fff => '.$unidadesss);die();
            // ControlStock($idproduct, true);
        ...
        if (! esportestandard($data['codigo'])) {
            ...
            */
            ...
            if ($idproduct != '') {
                ...
                sendmail2('llega referencia '.$referencia.' y no existe producto');
            }
        }
    }
...
function ProcesarValores($valores)
{
    ...
    switch ($tabla) {
        ...
        */
        ...
        case 'v_sinc_articulozonapostal':
            return 1;
            break;
...
try {
    ...
    $activofecha = ''.Db::getInstance()->getValue('SELECT activo FROM aalv_bandera_integracion WHERE activo=1 and date_add(fecha, INTERVAL 3 HOUR)<now()');
    if (($activo == 1) && ($activofecha == '')) {
        ...
        // if($_SERVER['HTTP_HOST'] == 'preproduccion.a-alvarez.com'){
        //     $URL = "http://192.168.1.3:59000/integracion/CambiosPendientes/?destino=2&limit=1";
        // } else if($_SERVER['HTTP_HOST'] != 'a-alvarez.com'){
        //     $URL = "http://223.1.1.22:9000/integracion/CambiosPendientes/?destino=2&limit=1";
        // } else {
        $URL = 'http://127.0.0.1:59000/integracion/CambiosPendientes/?destino=2&limit=500';
        // }

        $content = peticionget($URL);
        $jsonData = $content; // trim(stripslashes($content),'"');
        ...
        foreach ($results as $valores) {
            ...
            // if (in_array($valores["transaccion"], $exclude_transaccion)) {
            //     continue;
            // }
            $transaccion = $valores['transaccion'];
            $transaccionguia = $transaccion;
            echo 'transaccion pendiente: '.$transaccion.' total cambios: '.$valores['total_cambios'];

            $existe = ''.Db::getInstance()->getValue("select id from aalv_integracion_transacciones where id='".$transaccion."'");

            if ($existe == '') {

                echo 'llega....'.$transaccion.".....\n";

                Db::getInstance()->Execute("INSERT INTO aalv_integracion_transacciones(id, url_transaccion, fecha_creacion, primerid, total_cambios, fecha_confirmacion) VALUES ('".$valores['transaccion']."','".$valores['url_transaccion']."','".$valores['fecha_creacion']."',".$valores['primer_idcambiotabla'].','.$valores['total_cambios'].",'0000-00-00 00:00:00')");

                $tiempo_inicial = microtime(true);
                $contentitem = peticionget($valores['url_transaccion'].'&limit=5000');

                $tiempo_final = microtime(true);
                $tiempo = $tiempo_final - $tiempo_inicial;
                // addlog("Recuperarvalores transaccion: ".$tiempo);
                ...
                if ($count > 0) {
                    ...
                    $contarexito = 0;
                    $tieneperfilesnav = false;
                    $modeloperfilesnav = 0;

                    foreach ($objitem['results'] as $valoresitem) {

                        echo 'Procesando...'.$transaccion.' '.$valoresitem['tabla'].' '.$valoresitem['fila'].' '.$valoresitem['tipo']."\n";

                        if ($valoresitem['tabla'] == 'v_sinc_w_perfiles_nav') {
                            $tieneperfilesnav = true;

                            if ($valoresitem['tipo'] <= 2) {
                                $modeloperfilesnav = $valoresitem['data']['id_modelo'];
                            }
                        }

                        Db::getInstance()->Execute("INSERT INTO aalv_integracion_cambios(tabla, fila, data, tipo, transaccion, fecha_confirmacion) VALUES ('".$valoresitem['tabla']."',".$valoresitem['fila'].",'".pSQL(json_encode($valoresitem['data']))."','".$valoresitem['tipo']."','".$transaccion."','0000-00-00 00:00:00')");

                        // $contarexito=$contarexito+ProcesarValores($valoresitem);
                        ...
                        try {
                            ...
                            $tiempo_final = microtime(true);
                            $tiempo = $tiempo_final - $tiempo_inicial;
                            // addlog("ProcesarValores: ".$tiempo);

                        } catch (Exception $e) {

                            $d = new DateTime;
                            $stdout = fopen(dirname(__FILE__).'/integracionerrores.txt', 'a');
                            fwrite($stdout, $d->format("Y-m-d\TH:i:sP").' --- error '.$e->getMessage());
                            fwrite($stdout, "\n");
                            fwrite($stdout, ' --- error integracion '.$valoresitem['tabla'].' '.$valoresitem['fila'].' '.$valoresitem['tipo'].' '.$transaccion);
                            fwrite($stdout, "\n");
                            fclose($stdout);

                            sendmail('error '.$e->getMessage()."\nintegracion ".$valoresitem['tabla'].' '.$valoresitem['fila'].' '.$valoresitem['tipo'].' '.$transaccion);
                        }
                    }

                    if ($tieneperfilesnav) {
                        if ($modeloperfilesnav != 0) {
                            ProcesarPrincipal($modeloperfilesnav);
                        }
                    }
                    ...
                    while ($next != '') {
                        ...
                        $contentitem = peticionget($next);
                        $jsonDataitem = $contentitem; // trim(stripslashes($contentitem),'"');
                        $objitem = json_decode($jsonDataitem, true);
                    ...
                    if ($count == $contarexito) {
                        // confirmar transaccion http://127.0.0.1:59000/integracion/ConfirmarTransaccion/1.22.930292_2/
                        ...
                        // $tiempo_final = microtime(true);
                        // $tiempo = $tiempo_final - $tiempo_inicial;
                        // addlog("Confirmar transaccion: ". $transaccion. " tiempo:".$tiempo);
                        ...
                        // $tiempo_final = microtime(true);
                        // $tiempo = $tiempo_final - $tiempo_inicial;
                        // addlog("Update Confirmar transaccion: ". $transaccion. " tiempo:".$tiempo);

                        //     echo "contarexito => Transaccion " . $transaccion . " CONFIRMADA\n";
                        // } else {
                        //     echo "contarexito => Transaccion " . $transaccion . " NO CONFIRMADA\n";
                        //     //  sendmailhora("Transaccion ".$transaccion. " NO CONFIRMADA\n");
                        // }
                    }
                }
                echo "\nDatos totales:".$count.' datos con exito:'.$contarexito."\n";
            } else {

                echo "\nreintento ".$transaccion."\n";
                $urltransaccion = Db::getInstance()->getValue("select url_transaccion from aalv_integracion_transacciones where id='".$transaccion."'");

                $contentitem = peticionget($urltransaccion.'&limit=5000');
                ...
                if ($count > 0) {
                    ...
                    foreach ($objitem['results'] as $valoresitem) {
                        ...
                        $existecambio = ''.Db::getInstance()->getValue("select id from aalv_integracion_cambios where tabla='".$valoresitem['tabla']."' and fila=".$valoresitem['fila']." and tipo='".$valoresitem['tipo']."' and transaccion='".$transaccion."'");

                        if ($existecambio == '') {
                            Db::getInstance()->Execute("INSERT INTO aalv_integracion_cambios(tabla, fila, data, tipo, transaccion, fecha_confirmacion) VALUES ('".$valoresitem['tabla']."',".$valoresitem['fila'].",'".pSQL(json_encode($valoresitem['data']))."','".$valoresitem['tipo']."','".$transaccion."','0000-00-00 00:00:00')");
                        }
                        // if ($valoresitem["fila"]==100133370){
                        //    dump("llega0");
                        // }
                    ...
                    if ($count == $contarexito) {
                        ...
                        Db::getInstance()->Execute("Update aalv_integracion_cambios set fecha_confirmacion=now() where transaccion='".$transaccion."'");
                        Db::getInstance()->Execute("Update aalv_integracion_transacciones set fecha_confirmacion=now() where id='".$transaccion."'");
                        echo 'reintento => Transaccion '.$transaccion." CONFIRMADA\n";
                        // } else {
                        //     echo "reintento => Transaccion " . $transaccion . " NO CONFIRMADA\n";
                        //     // sendmailhora("Transaccion ".$transaccion. " NO CONFIRMADA\n");
                        // }
                    } else {

                        echo 'Exitos '.$contarexito."\n";
                        echo 'Transaccion '.$transaccion." NO CONFIRMADA\n";
            ...
            if ($objconfirm["estado"]=="confirmado"){
                Db::getInstance()->Execute("Update aalv_integracion_cambios set fecha_confirmacion=now() where transaccion='".$transaccion."'");
                Db::getInstance()->Execute("Update aalv_integracion_transacciones set fecha_confirmacion=now() where id='".$transaccion."'");
                echo "Transaccion ".$transaccion. " CONFIRMADA\n";
            }
            else{
                echo "Transaccion ".$transaccion. " NO CONFIRMADA\n";
        ...
        $tiempo_final_total = $tiempo_final_total - $tiempo_inicial_total;
        // $stdout = fopen(dirname(__FILE__) . '/logtiemposintegracion.txt', 'a');
        // fwrite($stdout, "Tiempo global: " . $tiempo_final_total . " Ultima transaccion: " . $transaccionguia);
        // fwrite($stdout, "\n");
        // fclose($stdout);
...
*/
    ...
    fwrite($stdout, $d->format("Y-m-d\TH:i:sP").' --- error '.$e->getMessage());
    fwrite($stdout, "\n");
    fwrite($stdout, ' --- error integracion ');
    fwrite($stdout, "\n");
    fclose($stdout);
    Db::getInstance()->Execute('UPDATE aalv_bandera_integracion set activo=0, fecha=now() WHERE id=1');
...
// ///////////////////////////////////////// ALSERNET /////////////////////////////////////////////////////
function TodoFeature($id_product, $data)
{
    for ($i = 0; $i < 3; $i++) {
        ...
        if (count($buscar_feature) != 0) {
            foreach ($buscar_feature as $keyy => $valuee) {
                ...
                if (count($buscar) == 0) {
                    ...
                    InsertFeature_value_lang($valuee['id_feature_value'], $data[$valuee['id_feature']]);
    ...
    foreach ($data as $key => $value) {
        AsignarTipoProducto($value, $id_product);
    }
}

/* Funciones Nesesaria para Agregar y editar los Feature
*/
    ...
    foreach ($buscar_feat as $keyy => $valuue) {
        switch ($valuue['id_feature']) {
            ...
            case '11':
                ...
                validarInpost($vall[$valuue['id_feature']], $id_product);
                break;
...
function AsignarTipoProducto($c_f_sf_g_asignado, $id_product)
{
    ...
    if ($buscar) {
        ...
        if (! $buscar_asignado) {
            // sendmailPruebas("Se agrego el tipo de pruducto al ID_PS ".$id_product." -> Tipo de producot: ".$buscar[0]['id_feature_value']." => ".$c_f_sf_g_asignado);
            Db::getInstance()->execute('insert into aalv_feature_product
                                    (id_feature,id_product,id_feature_value)
                                    value
...
// function ControlStock($id_product, $mail = false)
// {
...
//                                                 and pl.id_lang = 1");
//     if ($stock[0]['id_product_attribute']) {
//         $product_import = Db::getInstance()->ExecuteS("SELECT externo_disponibilidad, etiqueta
//                                                                    FROM aalv_combinaciones_import
//                                                                    WHERE id_product_attribute =" . $stock[0]['id_product_attribute']);
//         //if(ComparadorDeEtiquetas($id_product)){
//         if (strpos($product_import[0]['etiqueta'], 'CSW')) {
//             $CONTROL_STOCK_WEB = true;
//         }
//     } else {
//         $product_import = Db::getInstance()->ExecuteS("SELECT externo_disponibilidad, etiqueta
//                                                                    FROM aalv_combinacionunica_import
//                                                                    WHERE id_product =" . $stock[0]['id_product']);
//         //if(ComparadorDeEtiquetas($buscar[0]['id_product_attribute'], 'aalv_combinaciones_import', 'id_product_attribute')){
//         if (strpos($product_import[0]['etiqueta'], 'CSW')) {
//             $CONTROL_STOCK_WEB = true;
//         }
//     }

//     if (strpos($product_import[0]['etiqueta'], 'OCULTO WEB')) {
//         $product->active = false;
//         $product->update();
//         return;
//     }

//     if ($product_import[0]['externo_disponibilidad'] == 1) {
//         $product->active = true;
//         $product->update();
//         return;
//     }
...
//             //$product->active=false;
//             //Db::getInstance()->execute("update aalv_product set active = 0 where id_product = ".$id_product);
//             //Db::getInstance()->execute("update aalv_product_shop set active = 0 where id_product = ".$id_product);
//             //if($mail){
//             //sendmailPruebas("Se edito el Estado del Producto ".$id_product);
//             //}

//             //return $id_product;
//         } else {
//             $product->active = true;
//             //Db::getInstance()->execute("update aalv_product set active = 1 where id_product = ".$id_product);
//             //Db::getInstance()->execute("update aalv_product_shop set active = 1 where id_product = ".$id_product);
//         }
//     } else {
//         if ($stock[0]["total_stock"] <= '0') {
//             $product->active = false;
...
function ComparadorDeEtiquetas($product_y_attribute, $tabla = 'aalv_combinacionunica_import', $columna = 'id_product')
{

    // Buscamos las etiquetas que tiene cargadas el producto
    $etiquetas = Db::getInstance()->getValue('SELECT etiqueta FROM '.$tabla.' WHERE '.$columna.' = '.$product_y_attribute);
    $array_etiquetas = explode(',', $etiquetas);
    $array_etiquetas = array_map('trim', $array_etiquetas);

    // Buscamos las etiquetas que se ecluyen
    $etiquetas_excluir = Db::getInstance()->getValue('SELECT GROUP_CONCAT(etiqueta) from aalv_etiqueta_stock');
    $array_etiquetas_excluir = explode(',', $etiquetas_excluir);
    $array_etiquetas_excluir = array_map('trim', $array_etiquetas_excluir);

    // Se compara ambos resultas
    $interseccion = array_intersect($array_etiquetas_excluir, $array_etiquetas);

    /* RESULTADO
    *   TRUE => Tiene una etiqueta que se debe excluir
    *   FALSE => No tiene etiqueta que se debe excluir
    */
    return ! empty($interseccion);
}

function validarFeature($id_feature, $id_product)
{
...
function validarInpost($id, $id_product)
{
