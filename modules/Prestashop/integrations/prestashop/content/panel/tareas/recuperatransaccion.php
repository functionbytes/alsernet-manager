        ...
        if (count($product->getImages(1)) == 0) { // no tiene imagenes
            ...
            foreach ($image_arr as $image_val) {
                ...
                if (($image->validateFields(false, true)) === true &&
                    ...
                    if (! copyImg($product->id, $image->id, 'https://www.a-alvarez.com/productsimages/'.$image_val.'/1080/fill/ffffff', 'inventaries', true)) {
                        $image->delete();
                        echo 'pasa....1';
                    } else {
...
function ProcesarPerfilesNav($data, $fila, $tipo){
    ...
    if ($tipo<=2){
        ...
        $countcat = Db::getInstance()->getValue("SELECT count(id_cat) FROM aalv_category_import WHERE id_origen=". $data['id_valor']);



        if ($countcat==1){
            $idcatps = "".Db::getInstance()->getValue("SELECT id_cat FROM aalv_category_import WHERE id_origen=". $data['id_valor']);
            ...
            if ($countcat>1){
                ...
                $catimport = Db::getInstance()->ExecuteS("SELECT * FROM aalv_category_import WHERE id_origen=". $data['id_valor']);
                 ...
                 foreach($catimport as $catim){
                    ...
                    if (padreorigen($catim,$listaelementos)){
                        $idcatps = "".Db::getInstance()->getValue("SELECT id_cat FROM aalv_category_import WHERE id=". $data['id']);
        ...
        echo "prod: ".$idprodps." cat: ".$idcatps;



        //$idcatps="".Db::getInstance()->getValue("SELECT id_cat FROM aalv_category_import where id_origen=".$idvalor);
        ...
        if (($idprodps!="") && ($idcatps!="")){
            ...
            $idprodps="".Db::getInstance()->getValue("SELECT id_product FROM aalv_category_product_import WHERE fila=".$fila);
            $idcatps="".Db::getInstance()->getValue("SELECT id_cat FROM aalv_category_product_import WHERE fila=".$fila);
            Db::getInstance()->Execute("DELETE FROM aalv_category_product where id_category=".$idcatps." and id_product=".$idprodps);
            return	1;
...
function ProcesarProducto($data, $fila, $tipo)
{
    ...
    if ($idproductattributeps != '') {
        ...
        echo 'UPDATE aalv_combinaciones_import set unidades_oferta='.$data['unidades_oferta'].",etiqueta='".$data['etiqueta']."',estado_gestion=".$data['estado'].',es_segunda_mano='.$data['es_segunda_mano'].',externo_disponibilidad='.$data['externo_disponibilidad'].",codigo_proveedor='".$data['codigo_proveedor']."',precio_costo_proveedor=".$data['precio_costo_proveedor'].',tarifa_proveedor='.$data['tarifa_proveedor'].',es_arma='.$data['es_arma'].',es_arma_fogueo='.$data['es_arma_fogueo'].',es_cartucho='.$data['es_cartucho'].' WHERE id_product_attribute='.$idproductattributeps;
    ...
    if ($idprodpssinatributo != '') {
        ...
        Db::getInstance()->Execute('UPDATE aalv_combinacionunica_import set unidades_oferta='.$data['unidades_oferta'].",etiqueta='".$data['etiqueta']."',estado_gestion=".$data['estado'].',es_segunda_mano='.$data['es_segunda_mano'].',externo_disponibilidad='.$data['externo_disponibilidad'].",codigo_proveedor='".$data['codigo_proveedor']."',precio_costo_proveedor=".$data['precio_costo_proveedor'].',tarifa_proveedor='.$data['tarifa_proveedor'].',es_arma='.$data['es_arma'].',es_arma_fogueo='.$data['es_arma_fogueo'].',es_cartucho='.$data['es_cartucho'].' WHERE id_product='.$idprodpssinatributo);
        ...
        if ($tipo == 1) {
            ...
            if ($idprodps != '') {
                ...
                if ($existencombinaciones != 0) {
                    ...
                    Db::getInstance()->Execute('UPDATE aalv_combinaciones_import set unidades_oferta='.$data['unidades_oferta'].",etiqueta='".$data['etiqueta']."',estado_gestion=".$data['estado'].',es_segunda_mano='.$data['es_segunda_mano'].',externo_disponibilidad='.$data['externo_disponibilidad'].",codigo_proveedor='".$data['codigo_proveedor']."',precio_costo_proveedor=".$data['precio_costo_proveedor'].',tarifa_proveedor='.$data['tarifa_proveedor'].',es_arma='.$data['es_arma'].',es_arma_fogueo='.$data['es_arma_fogueo'].',es_cartucho='.$data['es_cartucho'].' WHERE id_product_attribute='.$idProductAttribute);
                    ...
                    if ($existeunica == '') {
                        ...
                        Db::getInstance()->Execute('UPDATE aalv_combinaciones_import set unidades_oferta='.$rowpasacomb['unidades_oferta'].",etiqueta='".$rowpasacomb['etiqueta']."',estado_gestion=".$rowpasacomb['estado_gestion'].',es_segunda_mano='.$rowpasacomb['es_segunda_mano'].',externo_disponibilidad='.$rowpasacomb['externo_disponibilidad'].",codigo_proveedor='".$rowpasacomb['codigo_proveedor']."',precio_costo_proveedor=".$rowpasacomb['precio_costo_proveedor'].',tarifa_proveedor='.$rowpasacomb['tarifa_proveedor'].',es_arma='.$rowpasacomb['es_arma'].',es_arma_fogueo='.$rowpasacomb['es_arma_fogueo'].',es_cartucho='.$rowpasacomb['es_cartucho'].' WHERE id_product_attribute='.$idProductAttribute);
...
function ProcesarValoresNavegacion($data, $fila, $tipo)
{
    ...
    if ($tipo <= 2) {
        ...
        if ($catps == '') {
            ...
            $catpadre = ''.Db::getInstance()->getValue('SELECT id_cat FROM aalv_category_import WHERE id_nav='.$idpadre);
            if ($catpadre != '') {
                $category = new Category((int) $catps);
                $category->id_parent = $catpadre;
                $category->update();
    ...
    if ($tipo <= 2) {
        ...
        if ($idproduct != '') {
            ...
            if ($idlote != '') {
                ...
                $idfeaturevalue = crearFeatureValue(6, '1', 1);
                if ($idfeaturevalue != 0) {
                    $product = new Product($idproduct);
...
$idcambio = Tools::getValue('id');

$valoresitem = Db::getInstance()->getRow('select tabla, fila, tipo, data from aalv_integracion_cambios where id='.$idcambio);
