...
function ExistePathCategory($producto, $id_nav)
{
    ...
    if (! escomun($id_nav) && ($id_nav != 0)) {
        ...
        if (''.$id_cat != '') {
            ...
            // echo "<br>existe $existe aaaa $elemento bbbb $id_padre cccccc $id_cat";
...
function ProcesarLllote($data, $fila, $tipo)
{
    ...
    if ($tipo <= 2) {
        ...
        if (''.$id_bundle_sub_product == '') {
            ...
            $idarticulo = $data['idarticulo'];
            //            $idproduct = Db::getInstance()->getValue("SELECT id_product FROM aalv_combinacionunica_import WHERE id_articulo=".$idarticulo." UNION select id_product from aalv_product_attribute where id_product_attribute in (SELECT id_product_attribute FROM aalv_combinaciones_import WHERE id_articulo = ".$idarticulo.") and id_product_attribute not in (SELECT id_product_attribute FROM aalv_tot_switch_attribute_disabled)");
            ...
            if ($bundlesection != '') {
                ...
                // $idproductattribute = "".Db::getInstance()->getValue("SELECT id_product_attribute FROM aalv_combinaciones_import WHERE id_articulo = ".$idarticulo. " and id_product_attribute not in (SELECT id_product_attribute FROM aalv_tot_switch_attribute_disabled)");
            ...
            echo '<br>articulo '.$data['idarticulo'];
...
*/
            ...
            echo '<br>prod '.$idproduct.' attr '.$idproductattribute;
            ...
            Db::getInstance()->Execute('UPDATE aalv_wk_bundle_sub_product set id_product='.$idproduct.', id_wk_bundle_section='.$bundlesection.' WHERE id_bundle_sub_product='.$id_bundle_sub_product);

            Db::getInstance()->Execute('UPDATE aalv_wk_bundle_sub_product_attribute set id_product='.$idproduct.' , id_product_attribute='.$idproductattribute.', default_attr='.$defaultattr.', id_wk_bundle_section='.$bundlesection.' WHERE id_bundle_sub_product_attribute='.$id_bundle_sub_product_attribute);
...
function ProcesarPrincipal($idmodelo)
{
    ...
    if ($idproducto != '') {
        ...
        $datos = ''.Db::getInstance()->getValue("SELECT data  FROM aalv_integracion_cambios WHERE tabla LIKE '%perfiles_nav%' AND data LIKE '%\"id_modelo\":".$idmodelo.",%' and data like '%true%'");
...
{
    $d = new DateTime;
    $stdout = fopen(dirname(__FILE__).'/modeloscatprod.txt', 'a');
    fwrite($stdout, $message);
    fwrite($stdout, "\n");
    fclose($stdout);
}

$filas = Db::getInstance()->ExecuteS("SELECT * FROM aalv_integracion_cambios WHERE tabla = 'v_sinc_w_perfiles_nav' order by id");

$modelos = [];
$modeloanterior = 0;
foreach ($filas as $valoresitem) {
    ...
    if ($valoresitem['tipo'] != 3) {
        $modelo = $valoresitem['data']['id_modelo'];
        echo ' id '.$valoresitem['id'].' modelo '.$modelo.' ';
    }
    ...
    if ($valoresitem['tipo'] != 3) {
        ...
        if ($modelo != $modeloanterior) {
            ...
            try {
                ProcesarPrincipal($modeloanterior);
