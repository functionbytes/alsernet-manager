...
try {
    ...
    if (($activo == 1) && ($activofecha == '')) {
        ...
        $URL = 'http://127.0.0.1:59000/integracion/CambiosPendientes/?destino=2&limit=7';
                ...
                if ($count > 0) {
                    ...
                    foreach ($objitem['results'] as $valoresitem) {
                        ...
                        try {
                            ...
                            $d = new DateTime;
                            $stdout = fopen(dirname(__FILE__).'/integracionprueba.txt', 'a');
                            fwrite($stdout, $d->format("Y-m-d\TH:i:sP").'Procesando 1 ...'.$transaccion.' '.$valoresitem['tabla'].' '.$valoresitem['fila'].' '.$valoresitem['tipo']);
                            fwrite($stdout, "\n");
                            fclose($stdout);
                    ...
                    while ($next != '') {
                        ...
                        foreach ($objitem['results'] as $valoresitem) {
                            ...
                            try {
                                ...
                                $d = new DateTime;
                                $stdout = fopen(dirname(__FILE__).'/integracionprueba.txt', 'a');
                                fwrite($stdout, $d->format("Y-m-d\TH:i:sP").'Procesando 2 ...'.$transaccion.' '.$valoresitem['tabla'].' '.$valoresitem['fila'].' '.$valoresitem['tipo']);
                                fwrite($stdout, "\n");
                                fclose($stdout);
