...
function procesarlineatransaccion($id)
{

    $valoresitem = Db::getInstance()->getRow('select * from aalv_integracion_cambios where id='.$id);
...
function restaurarprod($idarticulo)
{

    // v_sinc_w_producto
    // "idarticulo":100108611,

    // recuperar id_producto a partir del id_articulo
    $row = Db::getInstance()->getRow("SELECT fila,id FROM aalv_integracion_cambios WHERE tabla = 'v_sinc_w_producto' AND data LIKE '%\"idarticulo\":".$idarticulo.",%' and (tipo='1' or tipo='2') order by id desc");

    if ($row) {
        $id_trans = $row['id'];
        $id_producto = $row['fila'];

        procesarlineatransaccion($id_trans);

        $id_trans_stock = ''.Db::getInstance()->getValue("SELECT id FROM aalv_integracion_cambios WHERE tabla = 'v_sinc_stock_central_web' AND data LIKE '%\"idarticulo\":".$idarticulo.",%' order by id desc");
        if ($id_trans_stock != '') {
            procesarlineatransaccion($id_trans_stock);
        }

        $row = Db::getInstance()->getRow("SELECT fila,id FROM aalv_integracion_cambios WHERE tabla = 'v_sinc_tarifa_cabecera' AND data LIKE '%\"idarticulo\":".$idarticulo.",%' and (tipo='1' or tipo='2')  and data LIKE '%\"idregpais\":1,%' order by id desc");
        if ($row) {
            $id_trans_cabecera1 = $row['id'];
            $id_tarifa_cabecera1 = $row['fila'];

            procesarlineatransaccion($id_trans_cabecera1);

            $rowslineas = Db::getInstance()->ExecuteS("SELECT id FROM aalv_integracion_cambios WHERE tabla = 'v_sinc_tarifa_linea' AND data LIKE '%\"idtarifa_cabecera\":".$id_tarifa_cabecera1."%' and (tipo='1' or tipo='2') order by id ");

            foreach ($rowslineas as $linea) {
                $id_trans_linea = $linea['id'];
                procesarlineatransaccion($id_trans_linea);
            }

        }

        $row = Db::getInstance()->getRow("SELECT fila,id FROM aalv_integracion_cambios WHERE tabla = 'v_sinc_tarifa_cabecera' AND data LIKE '%\"idarticulo\":".$idarticulo.",%' and (tipo='1' or tipo='2')  and data LIKE '%\"idregpais\":2,%' order by id desc");
        if ($row) {
            $id_trans_cabecera2 = $row['id'];
            $id_tarifa_cabecera2 = $row['fila'];

            procesarlineatransaccion($id_trans_cabecera2);

            $rowslineas = Db::getInstance()->ExecuteS("SELECT id FROM aalv_integracion_cambios WHERE tabla = 'v_sinc_tarifa_linea' AND data LIKE '%\"idtarifa_cabecera\":".$id_tarifa_cabecera2."%' and (tipo='1' or tipo='2') order by id ");
