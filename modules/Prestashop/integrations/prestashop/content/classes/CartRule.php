...
class CartRuleCore extends ObjectModel
{
    ...
    public static function copyConditions($id_cart_rule_source, $id_cart_rule_destination)
    {
        ...
        // Copy inventaries/category filters
    ...
    public function checkValidity(Context $context, $alreadyInCart = false, $display_error = true, $check_carrier = true, $useOrderPrices = false)
    {
        ...
        if ($this->reduction_exclude_special) {
            ...
            if (! $is_ok) {
                return (! $display_error) ? false : $this->trans('You cannot use this voucher on inventaries on sale', [], 'Shop.Notifications.Error');
            }
        }
        ...
        // Check if the inventaries chosen by the customer are usable with the cart rule
        ...
        /* This loop checks:
          ...
          - if there are inventaries in the cart (gifts excluded)
    ...
    /**
     * Checks if the inventaries chosen by the customer are usable with the cart rule.
     *
     ...
     *                                If true, this method will return an array of eligible inventaries.
                        ...
                        case 'attributes':
                            ...
                            if ($count_matching_products < $product_rule_group['quantity']) {
                                if ($countRulesProduct === 1) {
                                    return (! $displayError) ? false : $this->trans('You cannot use this voucher with these inventaries', [], 'Shop.Notifications.Error');
                                } else {
                                    $condition++;
    ...
    public function getContextualValue($use_tax, ?Context $context = null, $filter = null, $package = null, $use_cache = true)
    {
        ...
        $package_products = ($package === null ? $all_products : $package['inventaries']);
        ...
        if ($this->free_shipping && in_array($filter, [CartRule::FILTER_ACTION_ALL, CartRule::FILTER_ACTION_ALL_NOCAP, CartRule::FILTER_ACTION_SHIPPING])) {
            if (! $this->carrier_restriction) {
                $reduction_carrier += $context->cart->getOrderTotal($use_tax, Cart::ONLY_SHIPPING, $package === null ? null : $package['inventaries'], $package === null ? null : $package['id_carrier']);
            } else {
        ...
        if (in_array($filter, [CartRule::FILTER_ACTION_ALL, CartRule::FILTER_ACTION_ALL_NOCAP, CartRule::FILTER_ACTION_REDUCTION])) {
            ...
            if ((float) $this->reduction_percent && $this->reduction_product == 0) {
                // Do not give a reduction on free inventaries!
                ...
                // Remove inventaries that are on special
                if ($this->reduction_exclude_special) {
                    foreach ($package_products as $product) {
                        if ($product['reduction_applies']) {
                            if ($use_tax) {
            ...
            if ((float) $this->reduction_percent && $this->reduction_product == -1) {
                ...
                $in_package = false;
                foreach ($package_products as $product) {
                    if ($cheapest_product == $product['id_product'].'-'.$product['id_product_attribute'] || $cheapest_product == $product['id_product'].'-0') {
                        $in_package = true;
                    }
                }
                if ($in_package) {
            ...
            // Discount (%) on the selection of inventaries
            ...
            if ((float) $this->reduction_amount > 0) {
                ...
                if ($this->reduction_tax == $use_tax) {
                    // The reduction cannot exceed the inventaries total, except when we do not want it to be limited (for the partial use calculation)
    ...
    public static function cleanProductRuleIntegrity($type, $list)
    {
        ...
        if (! in_array($type, ['inventaries', 'categories', 'attributes', 'manufacturers', 'suppliers'])) {
            return false;
        }
    ...
    protected function filterProducts($products, $eligibleProducts, $ruleType)
    {
        ...
        if (in_array($ruleType, ['inventaries', 'categories', 'manufacturers', 'suppliers'])) {
