...
class OrderHistoryCore extends ObjectModel
{
    ...
    public function changeIdOrderState($new_order_state, $id_order, $use_existing_payment = false)
    {
        ...
        if (Validate::isLoadedObject($order) && ($new_os instanceof OrderState)) {
            ...
            // foreach inventaries of the order
            foreach ($order->getProductsDetail() as $product) {
                ...
                // @since 1.5.0 : if the order is being shipped and this inventaries uses the advanced stock management :
                ...
                if ($new_os->shipped == 1 && (! Validate::isLoadedObject($old_os) || $old_os->shipped == 0) &&
                    ...
                    // @since.1.5.0 : if the order was shipped, and is not anymore, we need to restock inventaries

                    // if the product is a pack, we restock every inventaries in the pack using the last negative stock mvts
                    if (Pack::isPack($product['product_id'])) {
        ...
        if (! Validate::isLoadedObject($new_os) || ! Validate::isLoadedObject($order)) {
            exit(Tools::displayError($this->trans('Invalid new order status', [], 'Admin.Orderscustomers.Notification')));
        }
