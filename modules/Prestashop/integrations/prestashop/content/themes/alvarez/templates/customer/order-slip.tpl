...
{block name='page_title'}
  {l s='Credit slips' d='Shop.Customer.Orders'}
{/block}

{block name='page_content'}
  <h6>{l s='Credit slips you have received after canceled orders.' d='Shop.Customer.Orders'}</h6>
  {if $credit_slips}
    <table class="table table-striped table-bordered hidden-sm-down">
      <thead class="thead-default">
        <tr>
          <th>{l s='Order' d='Shop.Customer.Orders'}</th>
          <th>{l s='Credit slip' d='Shop.Customer.Orders'}</th>
          <th>{l s='Date issued' d='Shop.Customer.Orders'}</th>
          <th>{l s='View credit slip' d='Shop.Customer.Orders'}</th>
        </tr>
      </thead>
      <tbody>
    ...
    <div class="credit-slips hidden-md-up">
      {foreach from=$credit_slips item=slip}
        <div class="credit-slip">
          <ul>
            <li>
              <strong>{l s='Order' d='Shop.Customer.Orders'}</strong>
              <a href="{$slip.order_url_details}" data-link-action="view-order-details">{$slip.order_reference}</a>
            </li>
            <li>
              <strong>{l s='Credit slip' d='Shop.Customer.Orders'}</strong>
              {$slip.credit_slip_number}
            </li>
            <li>
              <strong>{l s='Date issued' d='Shop.Customer.Orders'}</strong>
              {$slip.credit_slip_date}
            </li>
            <li>
              <a href="{$slip.url}">{l s='View credit slip' d='Shop.Customer.Orders'}</a>
            </li>
          </ul>
        </div>
      {/foreach}
