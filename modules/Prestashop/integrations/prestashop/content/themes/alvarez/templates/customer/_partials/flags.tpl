{block name='product_flags'}
    ...
    {if isset($item.product.flag) && $item.product.flag|@count > 0}
        {assign var="flag_range" value=3-$total_extra_flag}
        {assign var="flag_slice" value=$item.product.flag|array_slice:0:$flag_range}
        <ul class="product-flags">
            {if !$item.product.segunda_mano}
                {if $item.product.hour}
    ...
    {else}
        {if $total_extra_flag != 0 && !$item.product.segunda_mano}
            <ul class="product-flags">
                {if $item.product.hour}
                    <li class="product-flag-item" style="color:#ffffff; background-color:#d6021f;">
                        {l s='Send 48 hour' d='Shop.Theme.Global'}
