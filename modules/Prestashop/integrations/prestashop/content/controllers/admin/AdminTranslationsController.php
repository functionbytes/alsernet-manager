...
class AdminTranslationsControllerCore extends AdminController
{
    ...
    protected function writeTranslationFile($override_file = false)
    {
        ...
        if ($fd = fopen($file_path, 'wb')) {
            ...
            ConfigurationKPI::updateValue('FRONTOFFICE_langs_EXPIRE', time());
    ...
    public function exportTabs()
    {
        ...
        $dir = _PS_langs_DIR_ . $this->lang_selected->iso_code . DIRECTORY_SEPARATOR;
        $path = $dir . 'tabs.php';
    ...
    public function submitExportLang()
    {
        if ($this->lang_selected->iso_code && $this->theme_selected) {
            ...
            $file_name = _PS_langs_DIR_ . '/export/' . $this->lang_selected->iso_code . '.gzip';
        ...
        if (!isset($_FILES['file']['tmp_name']) || !$_FILES['file']['tmp_name']) {
            ...
            if (Validate::isLangIsoCode($iso_code)) {
                ...
                foreach ($files_paths as $files_path) {
                    ...
                    if (is_dir(_PS_langs_DIR_ . '../' . $path) && !is_writable(_PS_langs_DIR_ . '../' . $path) && !in_array($path, $tmp_array)) {
                ...
                if ($error = $gz->extractList($files_paths, _PS_langs_DIR_ . '../')) {
                    if (is_object($error) && !empty($error->message)) {
                        ...
                        foreach ($files_list as $file2check) {
                            if (pathinfo($file2check['filename'], PATHINFO_BASENAME) == 'index.php' && file_put_contents(_PS_langs_DIR_ . '../' . $file2check['filename'], Tools::getDefaultIndexContent())) {
                                continue;
                            }
                        }
     ...
     * compare with global $_MODULES array and fill AdminTranslations::modules_langs array
    ...
    protected function findAndFillTranslations($files, $theme_name, $module_name, $dir = false)
    {
        ...
        // in AndminTranslation::modules_langs array
        // see below
        $array_check_duplicate = [];
        foreach ($files as $file) {
            if ((preg_match('/^(.*).tpl$/', $file) || preg_match('/^(.*).php$/', $file)) && Tools::file_exists_cache($file_path = $dir . $file)) {
                ...
                foreach ($matches as $key) {
                    ...
                    if (!in_array($module_key, $array_check_duplicate)) {
                        ...
                        if (!isset($this->modules_langs[$theme_name][$module_name][$template_name][$key]['trad'])) {
                            ++$this->total_expression;
                        }
                        if ($theme_name && array_key_exists($module_key, $GLOBALS[$name_var])) {
                            $this->modules_langs[$theme_name][$module_name][$template_name][$key]['trad'] = html_entity_decode($GLOBALS[$name_var][$module_key], ENT_COMPAT, 'UTF-8');
                        } elseif (array_key_exists($default_key, $GLOBALS[$name_var])) {
                            $this->modules_langs[$theme_name][$module_name][$template_name][$key]['trad'] = html_entity_decode($GLOBALS[$name_var][$default_key], ENT_COMPAT, 'UTF-8');
                        } else {
                            $this->modules_langs[$theme_name][$module_name][$template_name][$key]['trad'] = '';
                            ++$this->missing_langs;
                        }
                        $this->modules_langs[$theme_name][$module_name][$template_name][$key]['use_sprintf'] = $this->checkIfKeyUseSprintf($key);
                    }
                }
            }
        }
    ...
    public function getTranslationsInformations()
    {
        $this->translations_informations = [
            'back' => [
                ...
                'var' => '_LANGADM',
                'dir' => _PS_langs_DIR_ . $this->lang_selected->iso_code . '/',
                'file' => 'admin.php',
                'sf_controller' => true,
                'choice_theme' => false,
            ],
    ...
    public function renderKpis()
    {
        ...
        if (ConfigurationKPI::get('FRONTOFFICE_langs') !== false) {
            $helper->value = ConfigurationKPI::get('FRONTOFFICE_langs');
        }
        $helper->source = $this->context->link->getAdminLink('AdminStats') . '&ajax=1&action=getKpi&kpi=frontoffice_langs';
        $helper->refresh = (bool) (ConfigurationKPI::get('FRONTOFFICE_langs_EXPIRE') < $time);
        $kpis[] = $helper->generate();
    ...
    public function postProcess()
    {
        ...
        try {
            if (Tools::isSubmit('submitCopyLang')) {
                ...
                if ($this->access('edit')) {
                    // Only the PrestaShop team should write the translations into the _PS_langs_DIR_
                    if (!$this->theme_selected) {
    ...
    protected function redirect($save_and_stay = false, $conf = false, $modify_translation = false)
    {
        ...
        if ($modify_translation) {
            ...
            Tools::redirectAdmin($url_base . '&action=backups');
        }
    }
...
<body>
    ...
    public function initFormFront()
    {
                            ...
                            if (empty($key)) {
                                ...
                                if (isset($GLOBALS[$name_var][$prefix_key . '_' . md5($key)])) {
                                    ...
                                    if (!isset($new_lang[$key]['trad'])) {
                                        $new_lang[$key]['trad'] = '';
                                        if (!isset($missing_langs_front[$prefix_key])) {
                                            $missing_langs_front[$prefix_key] = 1;
                                        } else {
                                            ++$missing_langs_front[$prefix_key];
                                        }
                                    }
                                }
        ...
        $this->tpl_view_vars = array_merge($this->tpl_view_vars, [
            'missing_langs' => $missing_langs_front,
            'count' => $count,
    ...
    public function initFormBack()
    {
                        ...
                        if ($prefix_key && $stringToTranslate) {
                            if (isset($GLOBALS[$name_var][$prefix_key . md5($stringToTranslate)])) {
                                ...
                                if (!isset($tabs_array[$prefix_key][$stringToTranslate]['trad'])) {
                                    ...
                                    if (!isset($missing_langs_back[$prefix_key])) {
                                        $missing_langs_back[$prefix_key] = 1;
                                    } else {
                                        ++$missing_langs_back[$prefix_key];
                                    }
                                }
                            }
        ...
        $this->tpl_view_vars = array_merge($this->tpl_view_vars, [
            ...
            'mod_security_warning' => Tools::apacheModExists('mod_security'),
            'tabsArray' => $tabs_array,
            'missing_langs' => $missing_langs_back,
        ]);
    ...
    public function initFormFields()
    {
            ...
            if (isset($rules['validate'])) {
                foreach ($rules['validate'] as $key => $value) {
                    if (isset($GLOBALS[$name_var][$prefix_key . '_' . md5($key)])) {
                        ...
                        if (!isset($tabs_array[$prefix_key][$key]['trad'])) {
                            $tabs_array[$prefix_key][$key]['trad'] = '';
                            if (!isset($missing_langs_fields[$prefix_key])) {
                                $missing_langs_fields[$prefix_key] = 1;
                            } else {
                                ++$missing_langs_fields[$prefix_key];
                            }
                            ++$count;
                        }
                    }
        ...
        $this->tpl_view_vars = array_merge($this->tpl_view_vars, [
            ...
            'tabsArray' => $tabs_array,
            'cancel_url' => $this->context->link->getAdminLink('AdminTranslations'),
            'missing_langs' => $missing_langs_fields,
        ]);
    ...
    public function initFormModules()
    {
        ...
        if (!empty($modules)) {
            ...
            $this->tpl_view_vars = array_merge($this->tpl_view_vars, [
                ...
                'textarea_sized' => self::TEXTAREA_SIZED,
                'cancel_url' => $this->context->link->getAdminLink('AdminTranslations'),
                'modules_langs' => isset($this->modules_langs) ? $this->modules_langs : [],
                'missing_langs' => $this->missing_langs,
    ...
    public function initFormPDF()
    {
                ...
                foreach ($files as $file) {
                    if (!in_array($file, self::$ignore_folder) && Tools::file_exists_cache($file_path = $dir . $file)) {
                        if ($type == 'tpl') {
                            if (Tools::file_exists_cache($file_path) && is_file($file_path)) {
                                ...
                                foreach ($matches as $key) {
                                    if (isset($GLOBALS[$name_var][$prefix_key . md5($key)])) {
                                        ...
                                        if (!isset($tabs_array[$prefix_key][$key]['trad'])) {
                                            $tabs_array[$prefix_key][$key]['trad'] = '';
                                            if (!isset($missing_langs_pdf[$prefix_key])) {
                                                $missing_langs_pdf[$prefix_key] = 1;
                                            } else {
                                                ++$missing_langs_pdf[$prefix_key];
                                            }
                                        }
                                    }
                                    $tabs_array[$prefix_key][$key]['use_sprintf'] = $this->checkIfKeyUseSprintf($key);
                            ...
                            $tabs_array = $this->parsePdfClass($file_path, 'php', $GLOBALS[$name_var], $prefix_key, $tabs_array, $missing_langs_pdf);
                        }
                    }
                }
            }
        ...
        $this->tpl_view_vars = array_merge($this->tpl_view_vars, [
            ...
            'tabsArray' => $tabs_array,
            'cancel_url' => $this->context->link->getAdminLink('AdminTranslations'),
            'missing_langs' => $missing_langs_pdf,
        ]);
