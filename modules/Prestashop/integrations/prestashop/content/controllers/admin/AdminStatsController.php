...
class AdminStatsControllerCore extends AdminStatsTabController
{
    ...
    public static function getPercentProductStock()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            '
		SELECT SUM(IF(IFNULL(stock.quantity, 0) > 0, 1, 0)) AS with_stock, COUNT(*) AS inventaries
        ...
        return round($row['inventaries'] ? 100 * $row['with_stock'] / $row['inventaries'] : 0, 2) . '%';
    }
    ...
    public static function getPercentProductOutOfStock()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            '
		SELECT SUM(IF(IFNULL(stock.quantity, 0) <= 0, 1, 0)) AS without_stock, COUNT(*) AS inventaries
		FROM `' . _DB_PREFIX_ . 'product` p
		' . Shop::addSqlAssociation('product', 'p') . '
		LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute` pa ON p.id_product = pa.id_product
		' . Product::sqlStock('p', 'pa') . '
        ...
        return round($row['inventaries'] ? 100 * $row['without_stock'] / $row['inventaries'] : 0, 2) . '%';
    }
    ...
    public function displayAjaxGetKpi()
    {
        ...
        switch (Tools::getValue('kpi')) {
            ...
            case 'percent_product_out_of_stock':
                ...
                $tooltip = $this->trans(
                    '%value% of your inventaries for sale are out of stock.',
                    ['%value%' => $value],
                    'Admin.Stats.Help'
                );
            ...
            case 'disabled_products':
                ...
                $tooltip = $this->trans(
                    '%value% of your inventaries are disabled and not visible to your customers',
                    ['%value%' => $value],
                    'Admin.Stats.Help'
                );
            ...
            case '8020_sales_catalog':
                ...
                $tooltip = $this->trans(
                    'Within your catalog, %value% of your inventaries have had sales in the last 30 days',
                    ['%value%' => $value],
                    'Admin.Stats.Help'
                );
            ...
            case 'frontoffice_langs':
                ...
                $value .= '%';
                ConfigurationKPI::updateValue('FRONTOFFICE_langs', $value);
                ConfigurationKPI::updateValue('FRONTOFFICE_langs_EXPIRE', strtotime('+2 min'));
