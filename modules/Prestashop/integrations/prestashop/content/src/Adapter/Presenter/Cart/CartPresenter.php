...
class CartPresenter implements PresenterInterface
{
    ...
    public function iva($items, $lang, $event)
    {
        ...
        foreach ($items as $item) {
            foreach ($item['flag'] as $flag) {
                if ($flag['type'] == $eventType) {
                    array_push($ivaProducts, $item);
                    break;
                }
            }
        }

        /*$ivaProducts = array_filter($items, function ($item) {
            return isset($item['iva']) && $item['iva'] == true;
        });*/
        ...
        foreach ($ivaProducts as $product) {
            if (isset($product['total_wt']) && isset($product['quantity'])) {
                ...
                if ((int) $event['iva'] == 1) {
                    ...
                    $ivaRate = 21;
                    $ivaFactor = $ivaRate / 100;

                    $totalDiscount = $product['price_wt'] * $product['quantity'] - ($product['price_wt'] * $product['quantity'] / ($ivaFactor + 1));
                    ...
                    $ivaItems[] = [
                        ...
                        'discount_iva' => $totalDiscount,
                    ];

                    $totalDiscountIva += $totalDiscount;

                } else {
                    /** CODIGO PARA APLICAR UN DECUENTO EN BASE A TOTAL EJ 15 EUR" */
                    $totalDiscountIva += $product['price_wt'] * $product['quantity'];
                }
        ...
        if ((int) $event['iva'] == 1) {
            ...
            /** CODIGO PARA APLICAR UN DESCUENTO EN BASE A TOTAL EJ 15 EUR" */
            $total_regalo = 15;
            $minimo_para_regalo = 100;
            $multiplos = floor($totalDiscountIva / $minimo_para_regalo);
            $totalDiscountIva = $multiplos * $total_regalo;
            $totalDiscountIva = min($totalDiscountIva, $maxVoucherAmount);
        }

        return [
            'inventaries' => $ivaItems,
            'total_discount_iva' => $totalDiscountIva,
            'iva_message' => $event_text,
            'lang' => $lang == 1 || $lang == 4 ? 'true' : 'false',
        ];
    }
