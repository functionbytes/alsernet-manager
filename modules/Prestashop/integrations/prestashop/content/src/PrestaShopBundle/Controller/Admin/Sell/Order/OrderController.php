...
{
    /**
     * Default number of inventaries per page (in case invalid value is used)
     */
    public const DEFAULT_PRODUCTS_NUMBER = 8;

    /**
     * Options used for the number of inventaries per page
     */
    ...
    public function addProductAction(int $orderId, Request $request): Response
    {
        ...
        /**
         * Returning the inventaries list view is not required since we reload the whole list
    ...
    public function cancellationAction(int $orderId, Request $request)
    {
        ...
        try {
            ...
            if ($result->isSubmitted()) {
                if ($result->isValid()) {
                    $this->addFlash('success', $this->trans('Selected inventaries were successfully canceled.', 'Admin.Catalog.Notification'));
                } else {
                    $this->addFlashFormErrors($form);
                }
            }
    ...
    private function getErrorMessages(Exception $e)
    {
        ...
        return [
            ...
            ProductOutOfStockException::class => $this->trans(
                'There are not enough inventaries in stock.',
