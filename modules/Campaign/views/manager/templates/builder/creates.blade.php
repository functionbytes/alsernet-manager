...
@section('content')
    ...
    <div class="row">
        <div class="col-md-6">
            <form action="{{ route('Admin\TemplateController@builderCreate') }}" method="POST"
    ...
    <div class="row">
        <div class="col-md-12" style="position: relative;">
            <div class="d-flex align-items-center mt-4 template-create-sticky">
                <h3 class="text-semibold mr-auto mb-0 mt-0">Seleccione una de las plantillas base a continuación</h3>
                <div class="text-left">
                    <button class="btn btn-secondary start-design"><i class="icon-check"></i> Iniciar diseño</button>
                </div>
            </div>
            ...
            @foreach (\Modules\Campaign\Models\Template\TemplateCategory::all() as $category)
                @if ($category->templates()->count())
                    <div class="subsection pb-4">
                        ...
                        <div id="gallery-{{ $category->id }}" class="pb-4">
                            <div class="listing-form"
                                 data-url="{{ route('Admin\TemplateController@builderTemplates', [
                                    ...
                                    <div class="me-auto">
                                        <div class="filter-box">
											<span class="filter-group">
												...
												<button class="btn btn-xs sort-direction" rel="asc" data-popup="tooltip"
                                                        title="{{ trans('messages.change_sort_direction') }}"
                                                        role="button" class="btn btn-xs">
                    ...
                    <script>
                        $(function () {
                            var list = makeList({
                                url: '{{ route('manager.templates.builder.templates', [
									'category_uid' => $category->uid,
								]) }}',
