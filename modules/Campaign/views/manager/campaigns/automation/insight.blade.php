...
@include('theme.automation._tabs', ['tab' => 'insight'])
...
<div class="insight-stat-brief d-flex mt-3 mb-4">
    <a title="{{ trans('messages.automation.go_contacts') }}" href="javascript:;"
      onclick="timelinePopup.load('{{ route('Automation2Controller@contacts', [
        'uid' => $automation->uid,
        'type' => 'in_action',
    ...
    @foreach ($insight as $key => $element)
        ...
        <div class="mc-row d-flex align-items-center">
            ...
            <div class="flex-fill" style="width: 35%">
                <label title="{{ trans('messages.automation.go_contacts') }}" onclick="timelinePopup.load('{{ route('Automation2Controller@contacts', [
                    'uid' => $automation->uid,
                  ]) }}')" class="cursor-pointer xtooltip font-weight-semibold"
                >
                    {{ $action->getName() }}
                </label>
                <desc title="{{ trans('messages.automation.go_contacts') }}" onclick="timelinePopup.load('{{ route('Automation2Controller@contacts', [
                    'uid' => $automation->uid,
            ...
            <a
                ...
                href="javascript:;"
                onclick="timelinePopup.load('{{ route('Automation2Controller@timeline', [
                    'uid' => $automation->uid,
                  ]) }}')"
            ...
            <div class="flex-fill text-center">
                <h3 title="{{ trans('messages.automation.insight.percent_tip') }}" onclick="timelinePopup.load('{{ route('Automation2Controller@contacts', [
                    'uid' => $automation->uid,
                  ]) }}')" class="cursor-pointer xtooltip font-weight-semibold"
                >
