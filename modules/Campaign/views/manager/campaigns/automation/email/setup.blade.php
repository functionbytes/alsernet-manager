...
@section('content')
    ...
    @include('theme.automation.email._tabs', ['tab' => 'setup'])

    <h5 class="mb-3">Email Setup</h5>
    <p>{{ trans('messages.automation.email.setup.intro') }}</p>

    <form id="emailSetup" action="{{ route('Automation2Controller@emailSetup', $automation->uid) }}" method="POST">
        {{ csrf_field() }}
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                @include('helpers.form_control', [
                    ...
                    'rules' => $email->rules(),
                    'help_class' => 'email',
                    'url' => route('SenderController@dropbox'),
                    ...
                    'error' => trans('messages.sender.dropbox.error.' . Auth::user()->customer->allowUnverifiedFromEmailAddress(), [
                        'sender_link' => route('SendingDomainController@index'),
                    ]),
    ...
    <script>
        @if ($email->template)
            var EmailSetup = {
                ...
                getWebhooksPopup: function() {
                    if (this.webhooksPopup == null) {
                        this.webhooksPopup = new Popup({
                            url: '{{ route('Automation2Controller@webhooks', [
                                'email_uid' => $email->uid,
                            ]) }}',
                            onclose: function() {
                                EmailSetup.refresh();
                ...
                refresh: function() {
                    $.ajax({
                        url: "{{ route('Automation2Controller@emailSetup', [
                            'uid' => $automation->uid,
                            'email_uid' => $email->uid,
                        ]) }}",
                        method: 'GET',
