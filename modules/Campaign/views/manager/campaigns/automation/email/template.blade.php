...
@section('content')
    ...
    @include('theme.automation.email._tabs', ['tab' => 'template'])
        ...
        <div class="col-md-12">
            <div class="sub-section d-flex">
                    ...
                    <div class="media-left">
                        <div class="main">
                            ...
                            @if (in_array(Acelle\Model\Setting::get('builder'), ['both','pro']) && $email->template->builder)
                                <a href="{{ route('Automation2Controller@templateEdit', [
                                        'uid' => $automation->uid,
                                        'email_uid' => $email->uid,
                            ...
                            @if (in_array(Acelle\Model\Setting::get('builder'), ['both','classic']))
                                <a href="{{ route('Automation2Controller@templateEditClassic', [
                                        'uid' => $automation->uid,
                                        'email_uid' => $email->uid,
                            ...
                            @endif
                            <a href="{{ route('Automation2Controller@templateRemove', [
                                    'uid' => $automation->uid,
                                    'email_uid' => $email->uid,
                ...
                <div class="template-thumb-container">
                    ...
                    />
                    <a
                        onclick="popupwindow('{{ route('Automation2Controller@templatePreview', [
                            'uid' => $automation->uid,
                            'email_uid' => $email->uid,
                        ]) }}', `{{ $automation->name }}`, 800)"
                        href="javascript:;"
            ...
            <div class="sub-section">
                ...
                @include('theme.automation.email._attachment')
            </div>

            @include('theme.automation.email._preheader')

            <div class="text-end mt-5">
                <a href="javascript:;" onclick="sidebar.load(); popup.hide()" class="btn btn-inline-secondary mr-1">
                    {{ trans('messages.close') }}
                </a>
                <a href="javascript:;" onclick="popup.load('{{ route('Automation2Controller@emailConfirm', [
                    'uid' => $automation->uid,
                    'email_uid' => $email->uid
