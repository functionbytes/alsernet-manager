...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            @include('theme.automation.email.template._tabs', ['tab' => 'upload'])
            ...
            <form enctype="multipart/form-data" action="{{ route('Automation2Controller@templateUpload', [
                'uid' => $automation->uid,
                'email_uid' => $email->uid,
        ...
        $('.template-upload-form').submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    popup.load('{{ route('Automation2Controller@emailTemplate', [
                        'uid' => $automation->uid,
                        'email_uid' => $email->uid,
                    ]) }}');

                    builderSelectPopup.load('{{ route('Automation2Controller@templateBuilderSelect', [
                        'uid' => $automation->uid,
                        'email_uid' => $email->uid,
                    ]) }}');
