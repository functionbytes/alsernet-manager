<ul class="nav nav-tabs mt-3 mb-4 nav-underline">
    ...
    <li class="nav-item">
        <a class="nav-link insight" href="javascript:;" onclick="sidebar.load('{{ route('Automation2Controller@insight', $automation->uid) }}')">
    ...
    <li class="nav-item">
        @if ($automation->getTrigger()->getOption('type') == 'woo-abandoned-cart')
            <a href="javascript:;"
                onclick="timelinePopup.load('{{ route('Automation2Controller@cartStats', $automation->uid) }}')"
        ...
        @else
            <a href="javascript:;"
                onclick="timelinePopup.load('{{ route('Automation2Controller@contacts', $automation->uid) }}')"
                class="nav-link statistics">
                <i class="material-symbols-rounded me-2">multiline_chart</i>
                {{ trans('messages.automation.statistics') }}
            </a>
