...
@section('content')
        ...
        <div class="col-md-12">
			...
			<form id="trigger-select" action="{{ action("Automation2Controller@triggerSelect", $automation->uid) }}" method="POST" class="form-validate-jqueryz">
				...
				@if(View::exists('automation2.trigger.' . $key))
					@include('theme.automation.trigger.' . $key)
				@endif

				<button class="btn btn-secondary select-trigger-confirm mt-2"
					data-url="{{ route('Automation2Controller@triggerSelect', ['uid' => $automation->uid]) }}"
				>
    ...
    message: response.message
						...
						// Edit Trigger
						EditTrigger('{{ route('Automation2Controller@triggerEdit', $automation->uid) }}' + '?key=' + tree.getOptions().key);
					}, flag);
				}
			});
		}
