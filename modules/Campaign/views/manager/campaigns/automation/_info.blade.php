...
<div class="sidebar-header d-flex flex-center mb-2">
    ...
    <div class="d-flex align-items-center campaign-status-section">
        <span class="mr-3 small">{{ trans('messages.automation.status.' . $automation->status) }}</span>
        <label>
            @if(config('custom.japan') && !Acelle\Model\Setting::get('license'))
                <span license-required>
...
<script>
    ...
    $('[name="automation_status"]').change(function () {
        ...
        if (value) {
            url = '{{ route('Automation2Controller@enable', ["uids" => $automation->uid]) }}';
            confirm = '{!! trans('messages.automation.enable.confirm', ['name' => $automation->name]) !!}';
        } else {
            url = '{{ route('Automation2Controller@disable', ["uids" => $automation->uid]) }}';
