...
@section('content')
    ...
    @include('theme.automation._tabs_timeline', [
        'tab' => 'audience'
    ])

    <div class="contacts-stats-boxes d-flex align-items-center" style="pointer-events: none;">
        <div class="contacts-stats-box" onclick="timelinePopup.load('{{ route('Automation2Controller@contacts', [
            'uid' => $automation->uid,
            'type' => 'in_action',
        ]) }}')">
        ...
        <div class="contacts-stats-box skipped" onclick="timelinePopup.load('{{ route('Automation2Controller@contacts', [
            'uid' => $automation->uid,
            'type' => 'pending',
        ]) }}')">
        ...
        <div class="contacts-stats-box error" onclick="timelinePopup.load('{{ route('Automation2Controller@contacts', [
            'uid' => $automation->uid,
            'type' => 'error',
        ]) }}')">
            <label>0</label>
                    ...
                    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="btnGroupDrop1">
                        @if (config('app.demo'))
                            <a class="dropdown-item list-export-contacts"
                                href="{{ route('Automation2Controller@exportContacts', [
                                    'uid' => $automation->uid,
    ...
    <script>
        var listContact = makeList({
            url: '{{ route('Automation2Controller@contactsList', [
                'uid' => $automation->uid,
                'action_id' => request()->action_id,
                'type' => request()->type,
            ]) }}',
