    ...
    <div class="row">
        <div class="col-lg-12 d-flex align-items-stretch">
            ...
            <div class="card w-100">
                ...
                <form id="formFloorsEdit" action="{{ route('settings.warehouse.update',$warehouse->uid) }}" method="POST" role="form">

                    {{ csrf_field() }}

                    <div class="card-body">
                        <div class="d-flex no-block align-items-center">
                            <h5 class="mb-0">Editar almacén: {{ $warehouse->code }}</h5>
                        </div>
                        <p class="card-subtitle mb-3 mt-3">
                            Actualice los datos del almacén según sea necesario.
                        </p>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <select name="available" id="available" class="select2 form-control @error('available') is-invalid @enderror">
                                            ...
                                            <option value="1" {{ old('available', $warehouse->available ? '1' : '0') == '1' ? 'selected' : '' }}>Disponible</option>
                                            <option value="0" {{ old('available', $warehouse->available ? '1' : '0') == '0' ? 'selected' : '' }}>No disponible</option>
                                        </select>
                                    </div>
                                    @error('available')
                                    <span class="invalid-feedback">{{ $message }}</span>
