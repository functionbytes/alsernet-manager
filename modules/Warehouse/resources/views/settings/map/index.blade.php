...
<body>
    ...
    <script>
        ...
        const config = {
            ...
            locations: new Map(),
            routes: {
                locations: "{{ route('settings.warehouse.map.locations', $warehouse->uid) }}",
                update: "{{ route('settings.warehouse.map.update', $warehouse->uid) }}",
                create: "{{ route('settings.warehouse.map.create', $warehouse->uid) }}",
                delete: "{{ route('settings.warehouse.map.delete', ['warehouse_uid' => $warehouse->uid, 'location_uid' => ':id']) }}",
                sections: "{{ route('settings.warehouse.locations.sections.index', ['warehouse_uid' => $warehouse->uid, 'location_uid' => ':id']) }}"
            }
        };
