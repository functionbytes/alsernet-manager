...
$(document).ready(function() {
    ...
    $('#testPromptBtn').on('click', function() {
        ...
        $.ajax({
            url: '{{ route("manager.settings.suppliers.prompts.test") }}',
            method: 'POST',
            data: {
                template: template,
    ...
    $('#promptForm').on('submit', function(e) {
        ...
        const url = id
            ? '{{ route("manager.settings.suppliers.prompts.update", ":id") }}'.replace(':id', id)
            : '{{ route("manager.settings.suppliers.prompts.store") }}';
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = '{{ route("manager.settings.suppliers.prompts.index") }}';
                    }, 1000);
                }
            },
            error: function(xhr) {
