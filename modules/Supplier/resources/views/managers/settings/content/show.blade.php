...
@section('content')
    ...
    @include('managers.components.card', ['title' => 'Revisión de Contenido - ' . $content->product_name])
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item">
                        <a href="{{ route('manager.settings.suppliers.content.index') }}">
...
$(document).ready(function() {
    ...
    function saveEditedContent() {
        ...
        $.ajax({
            url: '{{ route("manager.settings.suppliers.content.update", ":id") }}'.replace(':id', contentId),
            method: 'PUT',
            data: {
    ...
    function performAction(action, actionName, notes = null) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '{{ route("manager.settings.suppliers.content.action", ":id") }}'.replace(':id', contentId),
                    method: 'POST',
                    data: {
                        action: action,
                        notes: notes
