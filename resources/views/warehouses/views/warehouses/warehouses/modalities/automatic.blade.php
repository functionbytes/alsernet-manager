...
@section('content')
    <div class="container-fluid note-has-grid inventaries-content">
            ...
            <input type="hidden" id="inventarie" name="inventarie"  value="{{$warehouse->uid}}">
            <input type="hidden" id="location" name="location"  value="{{$location->uid}}">
            <input type="hidden" id="location" name="location"  value="{{$section->uid}}">
            <input type="hidden" id="item" name="item"  value="{{$section->uid}}">
        ...
        <div class="card w-100">
            <div class="card-body">
                <div class="mb-4">
                    <h4 class="card-title fw-semibold">Ubicacion : {{$section->barcode}}</h4>
            ...
            $("#product").on('input', function() {
                ...
                if (product != '') {
                    ...
                    if (product.startsWith('100')) {
                        ...
                        $.ajax({
                            url: "{{ route('warehouse.warehouses.location.validate.product') }}",
                            headers: {
    ...
    $('.remove-product-btn').on('click', function() {
        ...
        if (confirmDelete) {
            ...
            $('#sendLocations').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('warehouse.warehouses.location.close') }}",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    type: "POST",
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            let url = "{{ route('warehouse.warehouses.arrange', [':inventarie']) }}".replace(':inventarie', inventarie);
