...
@section('content')
    ...
    @include('accountings.includes.card', ['title' => 'Gestionar Documento'])
    ...
    @include('accountings.components.alerts')
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            ...
            <div id="uploadSectionContainer">
                @include('accountings.views.documents.partials.upload-section', [
                    'document' => $document,
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btnSendMissingDocs').on('click', function() {
                ...
                // Enviar directamente
                $.ajax({
                    url: "{{ route('accounting.documents.send-missing', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    data: {
            ...
            $(document).on('click', '#btnSendRequestUpload', function() {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.send-notification', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    success: function(response) {
                        if (response.success) {
            ...
            $(document).on('click', '#btnSendReminder', function() {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.send-reminder', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    success: function(response) {
                        if (response.success) {
            ...
            $(document).on('click', '#confirmDeleteBtn', function() {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.delete-single', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    data: {
                        media_id: mediaId,
                        doc_type: docType
            ...
            $(document).on('click', '.confirm-upload-btn', function(e) {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.confirm-upload', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    success: function(response) {
                        if (response.success) {
            ...
            $('#confirmConfigBtn').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.update', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    data: configFormData,
                    success: function(response) {
                        if (response.success) {
            ...
            $(document).on('click', '#btnSendCustomEmail', function(e) {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.send-custom-email', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
            ...
            function reloadDocumentsSection(uid = documentUid) {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.refresh-section', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', uid),
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
            ...
            function reloadActionHistory(uid = documentUid) {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.refresh-action-history', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', uid),
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
            ...
            function performUpload($submitBtn, formData, $progressBar, $uploadStatus) {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.admin-upload', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            $('#btnSendUploadConfirmation').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.send-upload-confirmation', $document->uid) }}",
                    method: 'POST',
                    data: { notes: notes },
                    success: function(response) {
                        if (response.success) {
            ...
            $('#btnSendApproval').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.send-approval', $document->uid) }}",
                    method: 'POST',
                    data: {},
                    success: function(response) {
                        if (response.success) {
            ...
            $('#btnSendRejection').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('accounting.documents.send-rejection', $document->uid) }}",
                    method: 'POST',
                    data: {
                        reason: reason,
