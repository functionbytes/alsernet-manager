...
@section('content')
    ...
    @include('weapons.includes.card', ['title' => 'Gestionar Documento'])
    ...
    @include('weapons.components.alerts')
    ...
    <div class="row">
        <div class="col-lg-4">
            ...
            <!-- Document Notes -->
            @include('weapons.views.documents.includes.document-notes-sidebar')

            <!-- Action History -->
            <div id="actionHistoryContainer">
                @include('weapons.views.documents.includes.action-history')
            </div>

            <!-- Email History -->
            @include('weapons.views.documents.includes.email-history')

            <!-- Status Timeline -->
            @include('weapons.views.documents.includes.status-timeline')
        ...
        <div class="col-lg-8">
            ...
            <div id="uploadSectionContainer">
                @include('weapons.views.documents.partials.upload-section', [
                    'document' => $document,
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btnSendMissingDocs').on('click', function() {
                ...
                // Enviar directamente
                $.ajax({
                    url: "{{ route('weapons.documents.send-missing', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    data: {
            ...
            $(document).on('click', '#btnSendRequestUpload', function() {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.send-notification', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    success: function(response) {
                        if (response.success) {
            ...
            $(document).on('click', '#btnSendReminder', function() {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.send-reminder', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    success: function(response) {
                        if (response.success) {
            ...
            $(document).on('click', '#confirmDeleteBtn', function() {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.delete-single', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    data: {
                        media_id: mediaId,
                        doc_type: docType
            ...
            $(document).on('click', '.confirm-upload-btn', function(e) {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.confirm-upload', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    success: function(response) {
                        if (response.success) {
            ...
            $('#confirmConfigBtn').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.update', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    data: configFormData,
                    success: function(response) {
                        if (response.success) {
            ...
            $(document).on('click', '#btnSendCustomEmail', function(e) {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.send-custom-email', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
            ...
            function reloadDocumentsSection(uid = documentUid) {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.refresh-section', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', uid),
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
            ...
            function reloadActionHistory(uid = documentUid) {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.refresh-action-history', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', uid),
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
            ...
            function performUpload($submitBtn, formData, $progressBar, $uploadStatus) {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.admin-upload', ['uid' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', documentUid),
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            $('#btnSendUploadConfirmation').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.send-upload-confirmation', $document->uid) }}",
                    method: 'POST',
                    data: { notes: notes },
                    success: function(response) {
                        if (response.success) {
            ...
            $('#btnSendApproval').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.send-approval', $document->uid) }}",
                    method: 'POST',
                    data: {},
                    success: function(response) {
                        if (response.success) {
            ...
            $('#btnSendRejection').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('weapons.documents.send-rejection', $document->uid) }}",
                    method: 'POST',
                    data: {
                        reason: reason,
