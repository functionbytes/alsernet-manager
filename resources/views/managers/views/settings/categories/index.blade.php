...
@section('content')
    ...
    @include('managers.components.card', ['title' => 'Categorias'])
                        ...
                        <tr class="search-items">
                            ...
                            <td>
                                <span class="usr-email-addr"
                                      data-email="{{ $categorie->title }}">{{ Str::upper( Str::lower($categorie->title) )  }}</span>
                            </td>

                            <td>
                      <span class="badge {{ $categorie->available == 1 ? 'bg-light-secondary  text-primary' : 'bg-light-secondary  text-primary' }}  rounded-3 py-2 text-primary fw-semibold fs-2 d-inline-flex align-items-center gap-1">
                           ...
                           {{ $categorie->available == 1 ? 'Publico' : 'Oculto' }}
                            ...
                            <td>
                                <span class="usr-ph-no"
                                      data-phone="{{ date('Y-m-d', strtotime($categorie->updated_at)) }}">{{ date('Y-m-d', strtotime($categorie->updated_at)) }}</span>
                            </td>
                            <td class="text-left">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <li>
                                            <a class="dropdown-item d-flex align-items-center gap-3"
                                               href="{{ route('manager.categories.edit', $categorie->uid) }}">Editar</a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item d-flex align-items-center gap-3 confirm-delete"
                                               data-href="{{ route('manager.categories.destroy', $categorie->uid) }}">Eliminar</a>
                                        </li>
                                    </ul>
                                </div>
                            </td>
