...
@section('content')
    ...
    @include('managers.includes.card', ['title' => 'Editar Prompt'])
    ...
    <div class="widget-content searchable-container list">
        ...
        <form method="POST" action="{{ route('manager.settings.suppliers.prompts.update', $prompt->uid) }}">
            @csrf
            @method('PUT')
                    ...
                    <div class="card card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('label') is-invalid @enderror" name="label"
                                   value="{{ old('label', $prompt->label) }}" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('scope') is-invalid @enderror" name="scope" id="promptScope" required>
                                ...
                                <option value="global" {{ old('scope', $prompt->scope) === 'global' ? 'selected' : '' }}>Global (Todos)</option>
                                <option value="supplier" {{ old('scope', $prompt->scope) === 'supplier' ? 'selected' : '' }}>Por Proveedor</option>
                                <option value="category" {{ old('scope', $prompt->scope) === 'category' ? 'selected' : '' }}>Por Categoría</option>
                                <option value="source" {{ old('scope', $prompt->scope) === 'source' ? 'selected' : '' }}>Por Fuente</option>
                            </select>
                            @error('scope')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('priority') is-invalid @enderror" name="priority"
                                   value="{{ old('priority', $prompt->priority) }}" min="0" max="100">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="seo_focus">
                                <option value="0" {{ old('seo_focus', $prompt->seo_focus ? '1' : '0') === '0' ? 'selected' : '' }}>No</option>
                                <option value="1" {{ old('seo_focus', $prompt->seo_focus ? '1' : '0') === '1' ? 'selected' : '' }}>Sí</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="is_active">
                                <option value="1" {{ old('is_active', $prompt->is_active ? '1' : '0') === '1' ? 'selected' : '' }}>Activo</option>
                                <option value="0" {{ old('is_active', $prompt->is_active ? '1' : '0') === '0' ? 'selected' : '' }}>Inactivo</option>
                            </select>
                        </div>

                        <div class="alert alert-info py-2 px-3">
                            <small><strong>Versión:</strong> {{ $prompt->version }}</small>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card card-body">
                        ...
                        <div class="mb-3">
                            <textarea class="form-control font-monospace @error('prompt_template') is-invalid @enderror"
                                      ...
                                      rows="20" required>{{ old('prompt_template', $prompt->prompt_template) }}</textarea>
