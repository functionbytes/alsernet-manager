                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6 divShops d-none">
                                <div class="mb-3">
                                    <label class="control-label col-form-label">Tiendas/Almacenes</label>
                                    <div class="input-group">
                                        {!! Form::select('shop', $shops, null , ['class' => 'select2 form-control','id' => 'shop']) !!}
                                    </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            // Get role name from selected role option
            function getRoleNameFromId() {
                const roleId = $('#roles').val();
                if (!roleId) return null;
                return $('#roles option:selected').text().toLowerCase();
            }
            ...
            function roleRequiresShop() {
                ...
                // Get the role name text and check against the array
                const roleText = $('#roles option:selected').text();
                const roleNames = @json(array_keys($roles));

                for (let i = 0; i < roleNames.length; i++) {
                    const roleId = roleNames[i];
                    const roleData = $('#roles').find('option[value="' + roleId + '"]').text();
                    if (rolesRequiringAssignment.includes(roleData.toLowerCase())) {
                        if ($('#roles').val() == roleId) {
                            return true;
                        }
                    }
