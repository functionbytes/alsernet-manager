    ...
    <div class="widget-content searchable-container list">
        ...
        <div class="card card-body">
                ...
                <table class="table search-table align-middle text-nowrap">
                    ...
                    @foreach ($events as $key => $event)
                        ...
                        <tr class="search-items">
                            ...
                            <td>
                                <span class="badge {{ $event->available == 1 ? 'bg-light-secondary  text-primary' : 'bg-light-secondary  text-primary' }}  rounded-3 py-2 text-primary fw-semibold fs-2 d-inline-flex align-items-center gap-1">
                                           {{ $event->available == 1 ? 'Publico' : 'Oculto' }}
                                </span>
                            </td>

                            <td>
                                <span class="usr-ph-no" data-phone="{{ date('Y-m-d', strtotime($event->updated_at)) }}">{{ date('Y-m-d', strtotime($event->updated_at)) }}</span>
                            </td>
                            <td class="text-left">
            ...
            <div class="result-body ">
                <span>Mostrar {{ $events->firstItem() }}-{{ $events->lastItem() }} de {{ $events->total() }} resultados</span>
                <nav>
