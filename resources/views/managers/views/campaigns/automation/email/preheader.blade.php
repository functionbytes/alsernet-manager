...
@if ($email->preheader)
    <div class="p-3 border bg-light-secondary rounded-3">
    ...
    var EmailPreheader = class {
        ...
        constructor() {
            this.addPopup = new Popup({
                url: '{{ route('Automation2Controller@emailPreheaderAdd', [
                    'uid' => $automation->uid,
                    'email_uid' => $email->uid,
                ]) }}'
            });
        ...
        remove() {
            new Dialog('confirm', {
                ...
                ok: function() {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '{{ route('Automation2Controller@emailPreheaderRemove', [
                            'uid' => $automation->uid,
                            'email_uid' => $email->uid,
                        ]) }}',
                        data: {
